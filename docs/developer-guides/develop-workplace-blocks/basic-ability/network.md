---
document_id: '7180270043523088390'
directory_id: '7180165099250876421'
title: 网络
full_path: /uAjLw4CM/uYjL24iN/block/guide/basic-ability/network
breadcrumb:
- Developer Guides
...
source_url: https://open.larksuite.com/document/uAjLw4CM/uYjL24iN/block/guide/basic-ability/network
---
...
`tt.request` 的更多参数和用法参见:[request](/document/uAjLw4CM/uYjL24iN/block/api/network/request)。
## 注意事项
...
1.  不允许在小组件中直接调用Lark开放平台的接口（open.feishu.cn），应通过服务端调用。
...
1.  开发者服务器需要开启 CORS 以允许 https://*.feishu.cn 跨域访问。其中，`Access-Control-Allow-Origin` 的值不能包含星号，需要设置为确定的域名，可根据请求头部中的 Origin 字段确定。例如：
...
-   [数据协同](/document/uAjLw4CM/uYjL24iN/block/service/collaborative)
-   HTTP 长轮询
