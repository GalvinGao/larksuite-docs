---
document_id: '7348410416865771526'
directory_id: '7346117913857081349'
title: 步骤三（可选）：配置应用免登流程（ 旧版 ）
full_path: /uYjL24iN/uMTMuMTMuMTM/development-guide/old_webapp_login_process
breadcrumb:
- Developer Guides
- Deprecated Guides
- Web App
- Config the app login-free process (old version)
document_type: GuideDocumentType
updated_at: 2024-03-20T11:45:46Z
source_url: https://open.larksuite.com/document/uYjL24iN/uMTMuMTMuMTM/development-guide/old_webapp_login_process
---

# 步骤三（可选）：配置应用免登流程

在开放平台创建好自建应用后，即可在本地开发相应的 Web 项目。如果你希望应用在Lark客户端内被访问时，可以获取客户端登录用户的信息，实现免登访问应用，则可以为应用配置免登流程。本文主要介绍应用免登的工作流程，以及实现免登的 [requestAuthCode](/document/uYjL24iN/uUzMuUzMuUzM/20220308) 接口相关的常见问题。

:::note
**示例代码**
<br>
关于如何在 Web 项目中实现应用免登，开放平台提供了相应的网页应用开发教程。开发教程中包含完整的网页应用示例代码，你可以查看代码介绍、体验开发流程，从而帮助你了解和使用应用免登能力。详情参见[网页应用免登](/document/home/quickly-create-a-login-free-web-app/introduction-to-sample-code)。
:::
...
[requestAuthCode](/document/uYjL24iN/uUzMuUzMuUzM/20220308) 接口用于获取网页应用免登授权码，便于网页应用实现用户免登流程。在接口的`fail`返回对象中包含了 [errCode属性](/document/uYjL24iN/ukzNy4SO3IjL5cjM)（错误码和错误信息）。当你在实际开放过程中，接口错误码相应的排查建议如下表所示。
...
如果配置 URL 后，依然出现此错误码，则是因为你调用 [requestAuthCode](/document/uYjL24iN/uUzMuUzMuUzM/20220308) 接口的页面，不在重定向 URL 列表中。建议检查当前页面 URL 的完整路径（不包括`?`和`#`后面的部分），是否已经配置在开发者后台的应用详情页 **安全设置 > 重定向 URL** 中，如果没配置，则需要配置重定向 URL 后再试。
