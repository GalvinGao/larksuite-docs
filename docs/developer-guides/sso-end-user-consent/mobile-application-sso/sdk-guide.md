---
document_id: '7264784369457807365'
directory_id: '7073083801460473862'
title: SDK开发文档
full_path: /uAjLw4CM/uYjL24iN/mobile-app/sdk
breadcrumb:
- Developer Guides
...
updated_at: 2025-06-16T03:37:31Z
source_url: https://open.larksuite.com/document/uAjLw4CM/uYjL24iN/mobile-app/sdk
---
...
集成 Demo [LarkSSODemo 下载](https://sf16-sg.larksuitecdn.com/obj/open-platform-opendoc-sg/ark/1f51d4127f03df1f3ca8ea81e6d2be4c.zip)


### 配置信息
1. 配置跳转 Scheme: LSApplicationQueriesSchemes Key 下添加 lark 

![image.png](https://sf16-sg.larksuitecdn.com/obj/open-platform-opendoc-sg/4553c66ded039135835c43628d72295e_B8tFrWEMds.png?height=93&lazyload=true&width=1280)

2. 配置Lark回跳 URL：URL types Key 下添加注册时申请到到 app_id （例如：clia0988c0addf81013），注意需要去掉 app_id 中的下划线

![image.png](https://sf16-sg.larksuitecdn.com/obj/open-platform-opendoc-sg/de3993c98c48830f5913b3beed3878e6_G8zswWNW4u.png?height=223&lazyload=true&width=1280)

### API
#### 注册应用信息
```
LarkSSO.register(apps: [
  App(server: .lark, appId: "cli_a0988c0addf81013", scheme: "clia0988c0addf81013")
])
```

#### 设置语言

多语言目前会在手机没有安装Lark客户端的情况下，用于控制降级的H5页面的语言。Lark支持中、日、英三国语言，默认使用系统当前语言。
```
...
class AppDelegate: NSObject, UIApplicationDelegate {
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey : Any]? = nil) -> Bool {
        let app = App(server: .lark, appId: "12345", scheme: "12345")
...
class LarkSSOViewController: UIViewController, LarkSSODelegate {
    ...
    @objc func buttonTapped() {
        LarkSSO.send(request: .lark, viewController: self, delegate: self) // 发起授权
    }
...
```
## Android 接入指南
注：本文为LarkAndroid终端SDK新手使用教程，只涉及SDK的使用方法，默认读者已经具有使用Android Studio开发Android程序的经验，以及相关的编程知识基础等。
...
请到[Lark开放平台开发者后台](https://open.larksuite.com/app)进行申请，申请后将获得AppId，之后通过AppId为应用申请相关的权限，譬如授权；审核通过后即可使用相关的功能。

#### 下载Android Demo

[Lark授权demo](https://sf16-sg.larksuitecdn.com/obj/open-platform-opendoc-sg/ark/b8b7cc60592c91a15cea1272cfdac727.zip)提供了完整的工程配置及接入代码，你只需将其中的AppId替换为你申请的AppId，将local.properties中的sdk.dir配置为你的运行环境sdk位置，即可唤起Lark体验功能。

#### 集成到开发环境
- 下载[Lark授权sdk库](https://sf16-sg.larksuitecdn.com/obj/open-platform-opendoc-sg/ark/000a65be0abae79c418a24d7fa19c4a6.zip)；
...
|`setServer` | 设置Lark服务器，必须为`Lark` | 是 |
...
#### 常见问题
1. 在服务端注册的MD5签名如何获取？下载[GetMD5Signature.apk](https://sf16-sg.larksuitecdn.com/obj/open-platform-opendoc-sg/ark/GetMD5Signature.apk)，输入包名，获取MD5签名；复制MD5签名到剪贴板，该MD5签名仅做参考。
2. 错误类型有哪些？详见错误处理。

## 语言码
目前只用于降级H5页面显示配置，设置语言码（“zh”，“ja”，“jp” 等），默认使用系统当前语言。目前Lark支持中、日、英，Lark支持中、日、英等14种语言。
| 语言码 | 语言 |
|------|------|
|de| 德语|
|en| 英语|
...
## 开源声明
[larkLarkSSO Android open source notice file](https://sf16-sg.larksuitecdn.com/obj/open-platform-opendoc-sg/5a296964059bad63395139e81389a8ab_O57Ne29PQD.txt)
<br>
[larkLarkSSO_iOS open source notice file](https://sf16-sg.larksuitecdn.com/obj/open-platform-opendoc-sg/31cc7506246bacddfb75b4103202347c_Xf5nixGDlQ.txt)
