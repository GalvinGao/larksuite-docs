---
document_id: '6967302885512511493'
directory_id: '7278895853091241990'
title: 分享应用
full_path: /uAjLw4CM/ukzMukzMukzM/access-the-sharing-function
breadcrumb:
- Developer Guides
- Develop Process
...
updated_at: 2023-09-19T09:36:53Z
source_url: https://open.larksuite.com/document/uAjLw4CM/ukzMukzMukzM/access-the-sharing-function
---
...
- 如果你需要限制访问分享的成员范围、终端设备范围，请在应用内根据访问用户的[登录信息](/document/uYjL24iN/uYzMuYzMuYzM)、[终端设备信息](/document/uYjL24iN/uQjNx4CN2EjL0YTM)，补充必要的拦截提示。
:::

#### 分享渠道一：页面分享

通过实现小程序的生命周期函数[onShareAppMessage](/document/uYjL24iN/uQDNuQDNuQDN#c2e51fde)，你可以指定分享的标题、页面跳转路径和预览图。最终以卡片、链接或二维码的形式，将应用页面分享至Lark会话，或在Lark客户端外传播。
...
在实现了[onShareAppMessage](/document/uYjL24iN/uQDNuQDNuQDN#c2e51fde)的页面上，用户默认在小程序的容器菜单中可见当前页面的分享入口。你也可以通过 [hideShareMenu](/document/uYjL24iN/ukjN24SO2YjL5YjN)、[showShareMenu](/document/uYjL24iN/ugjN24CO2YjL4YjN)接口，控制分享入口在具体业务场景下的隐藏或恢复展示策略。
...
你可以在页面中，增加 [button](/document/uYjL24iN/uIjNuIjNuIjN) 组件，并将`open-type`设置为`share`，实现由按钮触发应用分享。
...
- 从Lark v3.40 版本开始，[share](/document/uYjL24iN/ugDM04COwQjL4ADN/thirdShare) 接口将严格校验请求参数中`url`的有效性。如果历史请求内容不是有效的 URL 地址，请注意及时调整适配。
:::

你可以在用户操作页面提供分享按钮，该按钮调用网页应用的 [share](/document/uYjL24iN/ugDM04COwQjL4ADN/thirdShare) 接口，指定分享卡片的标题、描述、配图以及跳转地址，最终以网页卡片的形式将应用发送给指定的Lark会话。
