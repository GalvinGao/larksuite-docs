---
document_id: '7273741028051042310'
directory_id: '7270783571969998854'
title: 示例代码说明
full_path: /home/quickly-develop-a-web-app-nodejs/sample-code-description
breadcrumb:
- Developer Guides
- Quick Starts
- Develop H5 (Node.js)
...
updated_at: 2023-09-01T06:36:25Z
source_url: https://open.larksuite.com/document/home/quickly-develop-a-web-app-nodejs/sample-code-description
---

# 示例代码说明

教程提供的示例代码完成了网页应用的免登和鉴权的前后端处理，本文主要介绍实现免登和鉴权处理的原理。
...
本示例代码作为应用的前端和服务端，建设了免登和鉴权处理，具体处理流程如下图所示：
- **免登流程**
  
  先获取应用授权凭证 [app_access_token](/document/ukTMukTMukTM/ukDNz4SO0MjL5QzM/auth-v3/auth/app_access_token_internal)，再综合去获取用户授权凭证 [user_access_token](/document/uAjLw4CM/ukTMukTMukTM/reference/authen-v1/access_token/create)。
  
  ![](//sf16-sg.larksuitecdn.com/obj/open-platform-opendoc-sg/fbb50e70795fcef72a8dc4a198a7505c_6T6suyu3ks.png?height=634&lazyload=true&width=924)

- **鉴权流程**

  先获取自建应用授权凭证 [tenant_access_token](/document/ukTMukTMukTM/ukDNz4SO0MjL5QzM/auth-v3/auth/tenant_access_token_internal)，再据此获取 [jsapi_ticket](/document/ukTMukTMukTM/uYTM5UjL2ETO14iNxkTN/h5_js_sdk/authorization)，然后计算签名 signature，最后通过 window.h5sdk.config 进行鉴权。
  
  ![](//sf16-sg.larksuitecdn.com/obj/open-platform-opendoc-sg/5cf59e28706562dce73c38f89b6aa15d_SQkXrAcxsu.png?height=794&lazyload=true&width=914)
...
async function getUserAccessToken(ctx) {
    ...
    //【请求】app_access_token：/ssl:ttdoc/ukTMukTMukTM/ukDNz4SO0MjL5QzM/auth-v3/auth/app_access_token_internal
    ...
    //【请求】user_access_token: /ssl:ttdoc/uAjLw4CM/ukTMukTMukTM/reference/authen-v1/access_token/create
...
- 时效性说明：用户信息中的 user_access_token 具有一定的时效性，默认最长有效期为 7200 秒，而服务端许多 API（例如，[文档类](/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-overview)）需要 user_access_token 作为访问凭证之一。如果你需要调用这类 API，服务端 Session 存储 user_access_token 保证在有效期内均可直接使用。如果期望刷新 user_access_token，可以参考[刷新 access_token](/document/uAjLw4CM/ukTMukTMukTM/reference/authen-v1/authen/refresh_access_token)。
...
async function getSignParameters(ctx) {
    ...
    //【请求】tenant_access_token：/ssl:ttdoc/ukTMukTMukTM/ukDNz4SO0MjL5QzM/auth-v3/auth/tenant_access_token_internal
    ...
    //【请求】jsapi_ticket：/ssl:ttdoc/ukTMukTMukTM/uYTM5UjL2ETO14iNxkTN/h5_js_sdk/authorization
