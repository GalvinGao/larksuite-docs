---
document_id: '7520565507028959244'
directory_id: '7520163178001612806'
title: 卡片 JSON 1.0 结构
full_path: /uAjLw4CM/ukzMukzMukzM/feishu-cards/card-json-structure
breadcrumb:
- Developer Guides
- Message cards
- Build card with JSON
- Card JSON 1.0 structure
document_type: GuideDocumentType
updated_at: 2025-06-27T09:54:00Z
source_url: https://open.larksuite.com/document/uAjLw4CM/ukzMukzMukzM/feishu-cards/card-json-structure
---

# 卡片 JSON 1.0 结构

本文档介绍卡片 JSON 1.0 的结构和属性。了解卡片 JSON 2.0 的结构和属性，参考[卡片 JSON 2.0 结构](/document/uAjLw4CM/ukzMukzMukzM/feishu-cards/card-json-v2-structure)。
...
```json
{
  ...
  "i18n_elements": {
    // 除标题外其它组件的多语言配置。你可在每个语种下配置完整卡片，卡片将根据用户的Lark客户端语言，自动展示对应语言的卡片内容。
    "en_us": [{}], // 英文
    "zh_cn": [{}], // 简体中文
    "zh_hk": [{}], // 繁体中文（香港）
    "zh_tw": [{}], // 繁体中文（台湾）
  ...
  "fallback": {
    ...
    "trigger_conditions": [
      ...
      {
        // 条件二：指定组件。当用户的Lark客户端版本低于这些组件支持的最低客户端版本时，触发降级。
        "type": "element_tags",
        "value": [
          ""
        ]
...
`header` 字段用于配置卡片的标题。了解`header` 字段说明，参见[标题组件](/document/uAjLw4CM/ukzMukzMukzM/feishu-cards/card-components/content-components/title)。
```json
  "header": {},  // 卡片标题
```
...
在卡片的`elements` 字段中，你需要添加卡片组件作为卡片正文内容，组件将按数组顺序纵向流式排列。了解卡片组件，参考[组件概述](/document/uAjLw4CM/ukzMukzMukzM/feishu-cards/card-components/component-overview)。
```json
...
Lark卡片支持多语言设置。设置多语言后，卡片将根据用户的Lark客户端语言，自动展示对应语言的卡片内容，满足国际化业务需求。本小节介绍多语言设置相关的 JSON 结构，详细步骤可参见[配置卡片多语言](/document/uAjLw4CM/ukzMukzMukzM/feishu-cards/configure-multi-language-content)。
...
```json
  "i18n_elements": {
    // 卡片正文的多语言配置。你可在每个语种下配置完整卡片，卡片将根据用户的Lark客户端语言，自动展示对应语言的卡片内容。
    "en_us": [{}], // 英文
    "zh_cn": [{}], // 简体中文
    "zh_hk": [{}], // 繁体中文（香港）
    "zh_tw": [{}], // 繁体中文（台湾）
...
在构建卡片时，若使用仅在较新版本客户端中支持的组件，那么在低版本的Lark客户端上，该组件将显示“请升级至Lark最新版本以查看内容”的提示。如果这类组件较多，不仅会丢失卡片的信息，还会降低卡片的视觉效果，影响用户体验。

为避免上述问题出现，你可以通过配置 `fallback` 字段为卡片添加以下降级规则，当满足以下任一规则时，将触发卡片降级并展示系统默认的卡片样式：
- 当用户的Lark客户端版本低于你指定的最低客户端版本时；
- 当用户的Lark客户端版本低于你指定的组件所支持的最低客户端版本时。

![](//sf16-sg.larksuitecdn.com/obj/open-platform-opendoc-sg/9c19366d1be9c5b883e5496c8fdbc9cb_5N2vMREvFS.png?height=316&lazyload=true&maxWidth=500&width=904)
