---
document_id: '7395083758943617029'
directory_id: '7393222083608526854'
title: 人员选择-多选
full_path: /uAjLw4CM/ukzMukzMukzM/feishu-cards/card-components/interactive-components/multi-select-user-picker
breadcrumb:
- Developer Guides
- Message cards
...
updated_at: 2025-06-27T09:55:27Z
source_url: https://open.larksuite.com/document/uAjLw4CM/ukzMukzMukzM/feishu-cards/card-components/interactive-components/multi-select-user-picker
---
...
本文档介绍人员选择-多选组件的 JSON 1.0 结构，要查看新版 JSON 2.0 结构，参考[人员选择-多选](/document/uAjLw4CM/ukzMukzMukzM/feishu-cards/card-json-v2-components/interactive-components/multi-select-user-picker)。
...
人员选择-多选组件支持Lark V7.4 及以上版本的客户端。在低于该版本的Lark客户端上，人员选择-多选组件的内容将展示为“请升级至最新版客户端以查看操作”。
## 嵌套规则

人员选择-多选组件仅支持嵌入在表单容器中使用，通过表单容器的“提交”按钮提交选择的内容。了解表单容器及其交互配置，参考[表单容器](/document/uAjLw4CM/ukzMukzMukzM/feishu-cards/card-components/containers/form-container)。
...
- 如果你添加的是新版卡片回传交互回调(`card.action.trigger`)，回调数据的结构如下所示。详细参数说明可参考[卡片回传交互](/document/uAjLw4CM/ukzMukzMukzM/feishu-cards/card-callback-communication)了解回调结构。
```json
{
    "schema": "2.0",
    "header": { 
...
```
- 如果你添加的是旧版卡片回传交互回调(`card.action.trigger_v1`)，回调数据的结构如下所示。详细参数说明可参考[消息卡片回传交互（旧）](/document/ukTMukTMukTM/uYzM3QjL2MzN04iNzcDN/configuring-card-callbacks/card-callback-structure)了解回调结构。
```json
{
