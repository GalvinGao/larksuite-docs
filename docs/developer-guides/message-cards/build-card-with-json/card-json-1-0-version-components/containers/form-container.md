---
document_id: '7395083758943469573'
directory_id: '7393222083608510470'
title: 表单容器
full_path: /uAjLw4CM/ukzMukzMukzM/feishu-cards/card-components/containers/form-container
breadcrumb:
- Developer Guides
- Message cards
...
updated_at: 2025-06-27T09:54:08Z
source_url: https://open.larksuite.com/document/uAjLw4CM/ukzMukzMukzM/feishu-cards/card-components/containers/form-container
---
...
本文档介绍表单容器的 JSON 1.0 结构，要查看新版 JSON 2.0 结构，参考[表单容器](/document/uAjLw4CM/ukzMukzMukzM/feishu-cards/card-json-v2-components/containers/form-container)。


![](//sf16-sg.larksuitecdn.com/obj/open-platform-opendoc-sg/a92dff5b6c098720a8909024c74078c1_VBz8XGXNPo.png?height=361&lazyload=true&maxWidth=500&width=947)

## 注意事项

- 表单容器支持Lark V6.6 及以上版本的客户端。在低于该版本的Lark客户端上，表单容器的内容将展示为“请升级至最新版本客户端，以查看内容”的占位图。
...
在[卡片 JSON 1.0 结构](/document/uAjLw4CM/ukzMukzMukzM/feishu-cards/card-json-structure)中：
  ...
  - 表单容器中不可直接内嵌[标签为 div 的组件](/document/ukTMukTMukTM/uYzM3QjL2MzN04iNzcDN/component-list/common-components-and-elements#6bdb3f37)。你可先内嵌分栏组件，再在分栏组件中内嵌标签为 `div` 类型的组件。
- 表单容器组件不可被内嵌在其它组件内，只可放在卡片根节点下。

在[卡片 JSON 2.0 结构](/document/uAjLw4CM/ukzMukzMukzM/feishu-cards/card-json-v2-structure)中：
...
当用户点击表单容器的提交按钮时，你在开发者后台配置的请求地址将会收到如下所示的回调数据。如果你添加的是新版卡片回传交互回调(`card.action.trigger`)，回调数据的结构如下所示。更多参数说明可参考[卡片回传交互](/document/uAjLw4CM/ukzMukzMukzM/feishu-cards/card-callback-communication)。
  ```json
  {
      "schema": "2.0", // 回调的版本
      "header": { // 回调基本信息
...
![](//sf16-sg.larksuitecdn.com/obj/open-platform-opendoc-sg/a92dff5b6c098720a8909024c74078c1_h4YqlevxIp.png?height=361&lazyload=true&maxWidth=500&width=947)
