---
document_id: '7395083758943518725'
directory_id: '6907567266541125634'
title: 配置卡片变量
full_path: /uAjLw4CM/ukzMukzMukzM/feishu-cards/feishu-card-cardkit/configure-card-variables
breadcrumb:
- Developer Guides
- Message cards
- Build card content
...
updated_at: 2024-07-24T08:05:00Z
source_url: https://open.larksuite.com/document/uAjLw4CM/ukzMukzMukzM/feishu-cards/feishu-card-cardkit/configure-card-variables
---
...
图片变量取值为调用[上传图片](/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/image/create)接口获取到的用于发送消息的图片 image_key，该变量用于动态展示图片。
...
- 如果开启了 **支持跳转**，则变量值的格式为：
    ...
    关于 url 元素的参数说明，参见 [url](/document/ukTMukTMukTM/uczNwUjL3cDM14yN3ATN)。
...
人员选项组变量通过用户 [open_id](/document/uAjLw4CM/ugTN1YjL4UTN24CO1UjN/trouble-shooting/how-to-obtain-openid) 来指定人员选项。以配置三个人员选项为例，变量格式如下：
...
循环对象数组作用于多列布局（column_set），并且需要与其他变量结合使用。该变量以数组的形式定义一个 column_set 中的所有变量，数组中每新增一个取值，在卡片内原有 column_set 的下方便会新增一个相同结构堆叠的 column_set。循环对象数组变量的示例格式如下所示。该部分代码需在发送卡片时传入。具体参考[使用卡片 ID 发送消息](/document/ukTMukTMukTM/uYzM3QjL2MzN04iNzcDN/send-message-card/send-message-using-card-id)。
...
本文介绍了卡片变量的相关配置信息，当你配置完成需要发送消息卡片时，开放平台的 OpenAPI 支持通过卡片模板 ID（template_id，即卡片 ID）、卡片模板的变量取值（template_variable）来发送消息卡片。具体操作参见[使用卡片 ID 发送消息](/document/ukTMukTMukTM/uYzM3QjL2MzN04iNzcDN/send-message-card/send-message-using-card-id)。
