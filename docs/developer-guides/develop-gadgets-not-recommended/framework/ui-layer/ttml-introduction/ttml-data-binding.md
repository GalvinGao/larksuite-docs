---
document_id: '6965379543684579334'
directory_id: '6907567266540453890'
title: 数据绑定
full_path: /uYjL24iN/uADOuADOuADO
breadcrumb:
- Developer Guides
- Develop Gadgets (Not Recommended)
- Framework
- UI Layer
- TTML Introduction
- TTML data binding
document_type: GuideDocumentType
updated_at: 2022-03-11T04:11:31Z
source_url: https://open.larksuite.com/document/uYjL24iN/uADOuADOuADO
---

# 数据绑定

## 基础绑定

```html
<!-- index.ttml -->
<view> {{ message }} </view>
```

```js
// index.js
Page({
  data: {
    message: 'Hello World!'
  }
})
```

## 属性绑定

```html
<!-- index.ttml -->
<!-- 双引号括住变量 -->
<view id="item-{{id}}"> </view>
```

```js
// index.js
Page({
  data: {
    id: 0
  }
})
```

## 指令属性(控制渲染逻辑)

```html
<!-- index.ttml -->
<view tt:if="{{condition}}"> </view>
```

```js
Page({
  data: {
    condition: true
  }
})
```

## 原生属性

```html
<!-- checked="false" 跟 checked="{{false}}" 表现一致-->
<checkbox checked="{{false}}"> </checkbox>
```

# 运算

可以在 {{}} 内进行简单的运算，支持的有如下几种方式：

## 三元运算

```html
<view hidden="{{flag ? true : false}}"> Hidden </view>
```

## 数运算

```html
<!-- index.ttml -->
<view> {{a + b}} + {{c}} + d </view>
```

```js
// index.js
Page({
  data: {
    a: 1,
    b: 2,
    c: 3
  }
})

// index.ttml 渲染结果是： 3 + 3 + d
```

## 字符串运算

```html
<!-- index.ttml -->
<view>{{"hello" + name}}</view>
```

```js
Page({
  data:{
    name: 'world'
  }
})
```

## 渲染对象属性

```html
<!-- index.ttml -->
<view>{{object.key}} {{array[0]}}</view>
```

```js
// index.js
Page({
  data: {
    object: {
      key: 'Hello '
    },
    array: ['World']
  }
})
```

## 逻辑控制

```html
<!-- index.ttml -->
<view tt:if="{{length > 5}}"> </view>
```

# 混合渲染

支持在 ttml 里面使用变量构建新结构。

## 数组

```html
<!-- index.ttml -->
<view tt:for="{{[zero, 1, 2, 3, 4]}}"> {{item}} </view>
```

```js
// index.js
Page({
  data: {
    zero: 0
  }
})
```

## 对象

```html
<!-- index.ttml -->
<template is="objectCombine" data="{{foo: a, bar: b}}"></template>
```

```js
// index.js
Page({
  data: {
    a: 1,
    b: 2
  }
})
```

也支持使用扩展运算符 `...` 将对象展开

```html
<!-- index.ttml -->
<template is="objectCombine" data="{{...obj1, ...obj2, e: 5}}"></template>
```

```js
// index.js
Page({
  data: {
    obj1: {
      a: 1,
      b: 2
    },
    obj2: {
      c: 3,
      d: 4
    }
  }
})

<!-- index.ttml 中data得到的赋值是： {a: 1, b: 2, c: 3, d: 4, e: 5} -->
```
