---
document_id: '7633446179720744375'
directory_id: '7641911776079302068'
title: 使用 Vercel Chat SDK 接入 Lark
full_path: /mcp_open_tools/integrating-agents-with-feishu/vercel-chat-sdk-lark-message-publish
breadcrumb:
- Lark CLI
- Integrate Agents into Lark Interactions
- Integrating Lark with Vercel Chat SDK
document_type: GuideDocumentType
updated_at: 2026-05-20T03:30:49Z
source_url: https://open.larksuite.com/document/mcp_open_tools/integrating-agents-with-feishu/vercel-chat-sdk-lark-message-publish
---

# 使用 Vercel Chat SDK 接入Lark
...
Lark已经为 Vercel Chat SDK 发布了官方适配器 [@larksuite/vercel-chat-adapter](https://www.npmjs.com/package/@larksuite/vercel-chat-adapter)，让 Chat SDK 上构建的产品可以无缝接入Lark。
...
- 消息编辑 / 删除 / 表情回复
- 历史消息拉取（覆盖Lark 20+ 种原生消息类型）
- 群聊 / 私聊识别 及 @ 提及处理
- WebSocket 长连接传输
- `registerLarkApp` 扫码授权一键创建应用

这个适配器基于Lark Channel SDK 构建（详见下方[适配器实现原理](https://open.larksuite.com/document/mcp_open_tools/integrating-agents-with-feishu/vercel-chat-sdk-lark-message-publish#16b76461)章节）。
...
调用 `registerLarkApp` 触发Lark官方扫码流程，生成一次性链接，用户扫码后自动返回 `client_id` / `client_secret`，并预配好本适配器所需的权限与事件订阅。
...
在 [open.larksuite.com/app](https://open.larksuite.com/app)（Lark）或 [open.feishu.cn/app](https://open.larksuite.com/app)（Lark）创建应用，获取 `client_id` / `client_secret`。
...
这个适配器基于 [Lark Channel SDK](https://github.com/larksuite/node-sdk)（`@larksuiteoapi/node-sdk` 中的 `LarkChannel`）构建。Channel SDK 是Lark提供的高阶 SDK，封装了：
...
整个适配器的核心控制流不到百行 —— Channel SDK 已经处理了大部分Lark平台特定的复杂度，适配器只承担"接口翻译"这一核心工作。
