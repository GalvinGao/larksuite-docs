---
document_id: '7026663896463835141'
directory_id: '7002892512470810630'
title: 示例代码介绍
full_path: /home/integrating-web-apps-in-5-minutes/debug-and-release
breadcrumb:
- Home
- Quickly Develop H5 (Python)
- Code explanation
document_type: GuideDocumentType
updated_at: 2023-09-01T08:29:54Z
source_url: https://open.larksuite.com/document/home/integrating-web-apps-in-5-minutes/debug-and-release
---
...
- 其他节点：服务端模块，使用 Flask 构建，主要功能如下。
    ...
    - 使用 jsapi_ticket、随机字符串、当前时间戳、当前鉴权的网页 URL 生成签名 signature。更多鉴权信息，可参见组件 SDK 鉴权流程。

## 代码解析

业务处理的逻辑图如下所示。

![](//sf16-sg.larksuitecdn.com/obj/open-platform-opendoc-sg/ark/a7a45ab6607d0392767198dd563fb21f.png?height=434&lazyload=true&width=1351)

### 服务端代码

1. 获取 access_token。
    
调用服务端 API 获取应用资源时，需要通过 access_token 来判断调用者身份。企业自建应用可通过[自建应用获取 tenant_access_token](/document/ukTMukTMukTM/ukDNz4SO0MjL5QzM/auth-v3/auth/tenant_access_token_internal) 接口获取。
...
jsapi_ticket 代表网页应用调用飞书 JSAPI 的临时凭证。你可利用上述获取的 access_token，调用获取 jsapi_ticket 接口获得 jsapi_ticket。

:::note
- 应用获取的 jsapi_ticket 存在有效期，其有效期为 2 小时。当你再次调用`获取 jsapi_ticket`接口时：
	
    - 如果 jsapi_ticket 剩余有效时间大于半小时，接口返回的 jsapi_ticket 和旧的 jsapi_ticket 值相同，且不续期。
	- 如果 jsapi_ticket 剩余有效时间小于半小时，接口返回一个新的 jsapi_ticket，与此同时旧的 jsapi_ticket 依然有效，到其原定期限过期。

- 由于获取 jsapi_ticket 的 API 调用次数有限，频繁刷新 jsapi_ticket 会导致 API 调用受限，影响自身业务，所以开发者在使用时需要缓存 jsapi_ticket （设置缓存过期时间为 2 小时），并不需要每次都从接口拉取。
...
<br>
如果你调用的 JSAPI 响应体中某个字段，在对应的开发文档中标注了 **字段权限要求**，表明此字段为敏感字段，仅当应用开通了对应的权限后才会在接口的响应体中返回此字段。如果无需获取这些字段，则不建议申请。权限申请的操作步骤，详情参见[申请 API 权限](/document/ukTMukTMukTM/uQjN3QjL0YzN04CN2cDN)。
...
- 除了部分特殊接口（例如，[requestAuthCode](/document/uYjL24iN/uUzMuUzMuUzM/20220308)、closeWindow），其他接口均需要鉴权成功后才可以调用。
