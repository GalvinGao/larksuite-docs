---
document_id: '7472964047861137414'
directory_id: '7472308035420635142'
title: 群公告概述
full_path: /uAjLw4CM/ukTMukTMukTM/group/upgraded-group-announcement/group-announcement-overview
breadcrumb:
- Server API
- Group Chat
...
updated_at: 2025-02-19T03:16:42Z
source_url: https://open.larksuite.com/document/uAjLw4CM/ukTMukTMukTM/group/upgraded-group-announcement/group-announcement-overview
---

# 群公告概述

本文档介绍Lark开放平台群组中的群公告能力相关的基本概念、使用限制、接入流程、方法列表等。


:::html
<md-alert type="tip">
本文档是针对新版群公告能力的说明。要了解新旧版本群公告能力的区别，参考[新旧版本说明](/document/uAjLw4CM/ukTMukTMukTM/group/upgraded-group-announcement/group-announcement-version-changes)。
</md-alert>
:::
...
群公告是群组中的公告文档，采用Lark云文档承载，每个群组只有一个群公告，每篇群公告都有唯一的 `chat_id`作为标识。要获取群的 `chat_id`，参考以下任一方式。
...
``` json
{
    ...
    "create_time": string,    // 群公告生成的时间戳（秒），数字串
    "update_time": string,    // 群公告更新的时间戳（秒），数字串
...
块与块之间可形成父子关系。在调用[在群公告中创建块](/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/chat-announcement-block-children/create)接口时，你需先了解以下规则，再指定父块和子块。
...
**子块规则**
  ...
  | 分栏列（GridColumn） | 只可通过[批量更新群公告块的内容](/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/chat-announcement-block/batch_update)接口中的 InsertGridColumnRequest 操作添加，不可直接作为子块添加。 |
  | 思维笔记（Mindnote） | 不支持。 |
  | 单元格（TableCell） | 只可通过[批量更新群公告块的内容](/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/chat-announcement-block/batch_update)接口中的 InsertTableRowRequest 或 InsertTableColumnRequest 操作添加，不可直接作为子块添加。 |
...
接入群公告 OpenAPI 的流程如下图所示。了解更多，参考[群组概述](/document/uAjLw4CM/ukTMukTMukTM/group/overview)的 **接入流程** 一节。
![](//sf16-sg.larksuitecdn.com/obj/open-platform-opendoc-sg/7e2c712313cbc2da9b298804cbcf94e2_mZ7KWsWpd5.png?height=214&lazyload=true&width=2276)
