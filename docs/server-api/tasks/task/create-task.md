---
document_id: '7256621408749879302'
directory_id: '7253088311760617478'
title: 创建任务
full_path: /uAjLw4CM/ukTMukTMukTM/task-v2/task/create
breadcrumb:
- Server API
- Tasks
- Task
...
updated_at: 2023-11-09T09:19:15Z
source_url: https://open.larksuite.com/document/uAjLw4CM/ukTMukTMukTM/task-v2/task/create
---
...
创建任务时，可以通过设置`members`字段来设置任务的负责人和关注人。关于member的格式，详见[功能概述](/document/uAjLw4CM/ukTMukTMukTM/task-v2/overview)中的“ 如何表示任务和清单的成员？ ”章节。

如果要设置任务的开始时间和截止时间，需要遵守任务时间的格式和约束。详见[功能概述](/document/uAjLw4CM/ukTMukTMukTM/task-v2/overview)中的“ 如何使用开始时间和截止时间？”章节。

如要设置自定义字段值，可以设置`custom_fields`字段。但因为自定义字段归属于清单，因此要填写的自定义字段的guid必须归属于要添加的清单(通过`tasklists`设置）。详见[自定义字段概览](/document/uAjLw4CM/ukTMukTMukTM/task-v2/custom_field/custom-field-overview)。

通过设置`client_token`实现幂等调用。详见[功能概述](/document/uAjLw4CM/ukTMukTMukTM/task-v2/overview)中的“ 幂等调用 ”章节。

如要创建一个任务的子任务，需要使用[创建子任务](/document/uAjLw4CM/ukTMukTMukTM/task-v2/task-subtask/create)接口。

创建任务时可以一并设置自定义字段值。但根据自定义字段的权限关系，任务只能添加`tasklists`字段设置的清单中关联的自定义字段的值。详见[自定义字段功能概述](/document/uAjLw4CM/ukTMukTMukTM/task-v2/custom_field/custom-field-overview)中的介绍。{尝试一下}(url=/api/tools/api_explore/api_explore_config?project=task&version=v2&resource=task&method=create)

:::html
<md-alert type="tip">
如果创建任务时要将任务加入到多个清单中，调用身份必须对所有清单有可编辑权限。详情见[清单功能概述](/document/uAjLw4CM/ukTMukTMukTM/task-v2/tasklist/overview)中的“清单是如何鉴权的？“章节。
</md-alert>
:::
...
<md-table>
  ...
  <md-tbody>
    <md-tr>
      ...
      <md-td>https://open.larksuite.com/open-apis/task/v2/tasks</md-td>
    </md-tr>
    <md-tr>
...
<md-dt-tr level="0">
	...
	<md-dt-td>
	任务截止时间。详见[功能概述](/document/uAjLw4CM/ukTMukTMukTM/task-v2/overview)中的“ 如何使用开始时间和截止时间？”章节。
...
<md-dt-tr level="0">
	...
	<md-dt-td>
	任务关联的第三方平台来源信息，用于来源信息在Lark任务界面的展示。只能创建任务时设置，一旦设置后就不可变更。详见[功能概述](/document/uAjLw4CM/ukTMukTMukTM/task-v2/overview)中的“ 如何使用Origin? ”章节。
	</md-dt-td>
</md-dt-tr>
...
详见[功能概述](/document/uAjLw4CM/ukTMukTMukTM/task-v2/overview)中的“ 如何表示任务和清单的成员？ ”章节。
...
<md-dt-tr level="0">
	...
	<md-dt-td>
	重复任务规则。如果设置，则该任务为“重复任务”。详见[任务功能概述](/document/uAjLw4CM/ukTMukTMukTM/task-v2/task/overview)中的“如何使用重复任务？”章节。
...
<md-dt-tr level="0">
	...
	<md-dt-td>
	任务自定义完成配置。详见[任务功能概述](/document/uAjLw4CM/ukTMukTMukTM/task-v2/task/overview)中的“ 如何使用任务自定义完成？”章节。
	</md-dt-td>
</md-dt-tr>
...
<md-dt-tr level="0">
	...
	<md-dt-td>
	幂等token。如果提供则触发后端实现幂等行为。详见[功能概述](/document/uAjLw4CM/ukTMukTMukTM/task-v2/overview)中的“ 幂等调用 ”章节。
...
<md-dt-tr level="0">
	...
	<md-dt-td>
	任务的开始时间(ms), 详见[功能概述](/document/uAjLw4CM/ukTMukTMukTM/task-v2/overview)中的“ 如何使用开始时间和截止时间？”章节。
	</md-dt-td>
</md-dt-tr>
...
<md-tr>
  ...
  <md-td>1470500</md-td>
  <md-td>创建任务失败。</md-td>
  <md-td>服务器内部错误。重试调用后持续出错可以联系支持。</md-td>
</md-tr>
...
<md-tr>
  ...
  <md-td>检查调用身份是否对任务要加入的清单有编辑权限。详见[清单是如何鉴权的？](/document/uAjLw4CM/ukTMukTMukTM/task-v2/faq)</md-td>
</md-tr>
