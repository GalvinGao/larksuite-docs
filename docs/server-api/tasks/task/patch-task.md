---
document_id: '7256621408749895686'
directory_id: '7253088311760617478'
title: 更新任务
full_path: /uAjLw4CM/ukTMukTMukTM/task-v2/task/patch
breadcrumb:
- Server API
- Tasks
- Task
...
updated_at: 2023-11-09T09:19:23Z
source_url: https://open.larksuite.com/document/uAjLw4CM/ukTMukTMukTM/task-v2/task/patch
---
...
更新时，将`update_fields`字段中填写所有要修改的任务字段名，同时在`task`字段中填写要修改的字段的新值即可。如果`update_fields`中设置了要变更一个字段的名字，但是task里没设置新的值，则表示将该字段清空。调用约定详情见[功能概述](/document/uAjLw4CM/ukTMukTMukTM/task-v2/overview)中的“ 关于资源的更新”章节。
...
如更新自定义字段的值，需要调用身份同时拥有任务的编辑权限和自定义字段的编辑权限。详情见[自定义字段功能概览](/document/uAjLw4CM/ukTMukTMukTM/task-v2/custom_field/custom-field-overview)。更新时，只有填写在`task.custom_fields`的自定义字段值会被更新，不填写的不会被改变。

任务成员/提醒/清单数据不能使用本接口进行更新。
* 如要修改任务成员，需要使用[添加任务成员](/document/uAjLw4CM/ukTMukTMukTM/task-v2/task/add_members)
和[移除任务成员](/document/uAjLw4CM/ukTMukTMukTM/task-v2/task/remove_members)接口。
* 如要修改任务提醒，需要使用[添加任务提醒](/document/uAjLw4CM/ukTMukTMukTM/task-v2/task/add_reminders)和[移除任务提醒](/document/uAjLw4CM/ukTMukTMukTM/task-v2/task/remove_reminders)接口。
* 如要变更任务所在的清单，需要使用[任务加入清单](/document/uAjLw4CM/ukTMukTMukTM/task-v2/task/add_tasklist)和[任务移出清单]( /document/uAjLw4CM/ukTMukTMukTM/task-v2/task/remove_tasklist)接口。{尝试一下}(url=/api/tools/api_explore/api_explore_config?project=task&version=v2&resource=task&method=patch)

:::html
<md-alert type="tip">
修改时需要调用身份对任务有编辑权限。详情见[任务功能概述](/document/uAjLw4CM/ukTMukTMukTM/task-v2/task/overview)中的“任务是如何鉴权的？”章节。
</md-alert>
:::
...
<md-table>
  ...
  <md-tbody>
    <md-tr>
      ...
      <md-td>https://open.larksuite.com/open-apis/task/v2/tasks/:task_guid</md-td>
    </md-tr>
    <md-tr>
...
<md-dt-tr level="1">
	...
	<md-dt-td>
	如果设置，则该任务为“重复任务”。详见[任务功能概述](/document/uAjLw4CM/ukTMukTMukTM/task-v2/task/overview)中的“如何使用重复任务？”章节。
...
<md-enum-item key="description" >任务描</md-enum-item>
...
<md-tr>
  ...
  <md-td>检查调用身份是否对任务有编辑权限。详情见[任务功能概述](/document/uAjLw4CM/ukTMukTMukTM/task-v2/task/overview)中的“任务是如何鉴权的？”章节。</md-td>
</md-tr>
