---
document_id: '7256621408749617158'
directory_id: '7253088311760617478'
title: 任务功能概述
full_path: /uAjLw4CM/ukTMukTMukTM/task-v2/task/overview
breadcrumb:
- Server API
- Tasks
- Task
...
updated_at: 2023-11-09T09:19:11Z
source_url: https://open.larksuite.com/document/uAjLw4CM/ukTMukTMukTM/task-v2/task/overview
---
...
使用[创建任务](/document/uAjLw4CM/ukTMukTMukTM/task-v2/task/create)接口创建一个任务。创建的同时，可以添加任务的初始成员，开始/截止时间，提醒时间等，也可以在创建的任务的同时将其加入1个或多个清单。

使用[获取任务详情](/document/uAjLw4CM/ukTMukTMukTM/task-v2/task/get)接口可以获取到一个任务的全部数据。

使用[删除任务](/document/uAjLw4CM/ukTMukTMukTM/task-v2/task/delete)接口可以删除一个任务。对删除的任务无法做任何操作。已删除的任务也无法恢复。

使用[列取任务列表](/document/uAjLw4CM/ukTMukTMukTM/task-v2/task/list)接口可以获取调用者负责的所有任务。

使用[更新任务](/document/uAjLw4CM/ukTMukTMukTM/task-v2/task/patch)接口可以修改一个任务的属性，包括标题，描述，截止时间等。但对于任务成员、提醒时间和加入/移出清单有单独的接口。
...
如要变更任务所在的清单，需要使用[任务加入清单](/document/uAjLw4CM/ukTMukTMukTM/task-v2/task/add_tasklist)和[任务移出清单]( /document/uAjLw4CM/ukTMukTMukTM/task-v2/task/remove_tasklist)接口。可以通过[列取任务所在清单](/document/uAjLw4CM/ukTMukTMukTM/task-v2/task/tasklists)接口查看一个任务所在的所有清单。

如需要变更一个任务的依赖，可以使用[添加依赖](/document/uAjLw4CM/ukTMukTMukTM/task-v2/task/add_dependencies)和[移除依赖](/document/uAjLw4CM/ukTMukTMukTM/task-v2/task/remove_dependencies)接口。
...
将任务设置为”自定义完成“的方式是设置任务的`custom_complete`字段。该字段可以在[创建任务](/document/uAjLw4CM/ukTMukTMukTM/task-v2/task/create)时设置，也可以通过[更新任务](/document/uAjLw4CM/ukTMukTMukTM/task-v2/task/patch)接口修改。
...
由于Lark任务只支持RRULE规范的子集，Lark任务并不能保证可以支持任意符合RFC5545规范的RRULE字符串。如果要构造一个Lark任务可用的`repeat_rule`，一个简单的方式是在Lark任务的界面上设置一个重复规则，然后调用[获取任务详情接口](/document/uAjLw4CM/ukTMukTMukTM/task-v2/task/get)得到其`repeat_rule`字符串。
