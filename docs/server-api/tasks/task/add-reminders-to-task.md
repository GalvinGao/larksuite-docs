---
document_id: '7256621408749748230'
directory_id: '7253088311760617478'
title: 添加任务提醒
full_path: /uAjLw4CM/ukTMukTMukTM/task-v2/task/add_reminders
breadcrumb:
- Server API
- Tasks
- Task
...
updated_at: 2023-11-09T09:19:54Z
source_url: https://open.larksuite.com/document/uAjLw4CM/ukTMukTMukTM/task-v2/task/add_reminders
---

# 添加任务提醒

为一个任务添加提醒。提醒是基于任务的截止时间计算得到的一个时刻。为了设置提醒，任务必须首先拥有截止时间(due)。可以在[创建任务](/document/uAjLw4CM/ukTMukTMukTM/task-v2/task/create)时设置截止时间，或者通过[更新任务](/document/uAjLw4CM/ukTMukTMukTM/task-v2/task/patch)设置一个截止时间。

目前一个任务只能设置1个提醒。但接口的形式可以在未来扩充为一个任务支持多个提醒。

如果当前任务已经有提醒了，要更新提醒的设置，需要先调用[移除任务提醒](/document/uAjLw4CM/ukTMukTMukTM/task-v2/task/remove_reminders)接口移除原有提醒。再调用本接口添加提醒。{尝试一下}(url=/api/tools/api_explore/api_explore_config?project=task&version=v2&resource=task&method=add_reminders)

:::html
<md-alert type="tip">
需要任务的编辑权限。详情见[任务功能概述](/document/uAjLw4CM/ukTMukTMukTM/task-v2/task/overview)中的“任务是如何鉴权的？”章节。
</md-alert>
:::
...
| 基本 |  |
| --- | --- |
| HTTP URL | https://open.larksuite.com/open-apis/task/v2/tasks/:task_guid/add_reminders |
| HTTP Method | POST |
...
| 400 | 1470613 | 任务已经有提醒了。 | 需要先调用[移除任务提醒](/document/uAjLw4CM/ukTMukTMukTM/task-v2/task/remove_reminders)接口移除原有提醒。再调用本接口添加提醒。 |
| 403 | 1470403 | 缺少任务编辑权限。 | 检查调用身份是否拥有任务编辑权限。详情见[任务功能概述](/document/uAjLw4CM/ukTMukTMukTM/task-v2/task/overview)中的“任务是如何鉴权的？”章节。 |
