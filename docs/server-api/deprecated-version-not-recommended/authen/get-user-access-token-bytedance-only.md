---
document_id: '7520163088822845446'
directory_id: '7345445193132883974'
title: 获取 user_access_token（仅字节）
full_path: /uAjLw4CM/ukTMukTMukTM/authentication-management/access-token/get-user_access_token-bytedance
breadcrumb:
- Server API
...
updated_at: 2025-06-26T07:53:44Z
source_url: https://open.larksuite.com/document/uAjLw4CM/ukTMukTMukTM/authentication-management/access-token/get-user_access_token-bytedance
---

# 获取 user_access_token（仅字节）
:::html
<md-alert type="error">
本接口已成为历史版本，不推荐使用。请使用最新版本：[获取 user_access_token ](/document/uAjLw4CM/ukTMukTMukTM/authentication-management/access-token/get-user-access-token)
</md-alert>。
:::

根据[登录预授权码](/document/common-capabilities/sso/api/obtain-oauth-code) 返回 code 获取 `user_access_token`。{尝试一下}(url=/api/tools/api_explore/api_explore_config?project=authen&version=v1&resource=oidc.access_token&method=create)
...
- 为了让流程更加规范，本接口不再返回用户信息，只返回token相关的字段。如需用户信息，请通过 [获取用户信息](/document/uAjLw4CM/ukTMukTMukTM/reference/authen-v1/user_info/get)接口获取数据
</md-alert>
:::
...
<md-table>
  ...
  <md-tbody>
    <md-tr>
      ...
      <md-td>https://open.larksuite.com/open-apis/authen/v1/oidc/access_token</md-td>
    </md-tr>
    <md-tr>
...
<md-dt-tr level="0">
	...
	<md-dt-td>
	登录预授权码，调用[登录预授权码](/document/common-capabilities/sso/api/obtain-oauth-code) 获取code
...
2. 确保[获取登录授权码 code](/document/common-capabilities/sso/api/obtain-oauth-code) 时，设置的回调地址 redirect_uri 参数，已配置到开发者后台 > 应用详情页 > 安全设置 > 重定向 URL。

关于该报错的详细解决方案，参见[如何解决授权免登页面 20029 错误](/document/uAjLw4CM/ugTN1YjL4UTN24CO1UjN/trouble-shooting/how-to-resolve-the-authorization-page-20029-error)。</md-td>
</md-tr>
...
<md-tr>
  ...
  <md-td>应用品牌和域名品牌不一致，请保证lark应用在lark域名下使用</md-td>
</md-tr>
