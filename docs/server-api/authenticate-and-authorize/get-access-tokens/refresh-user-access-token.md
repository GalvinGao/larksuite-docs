...
<md-table>
  ...
  <md-tbody>
    <md-tr>
      ...
      <md-td>https://open.larksuite.com/open-apis/authen/v2/oauth/token</md-td>
    </md-tr>
    <md-tr>
...
**示例值：**`cli_a5ca35a685b0x26e`
    ...
    <md-tr level="0">
      ...
      <md-td> 是</md-td>
      <md-td>
应用的 App Secret，可以在开发者后台中的应用详情页面找到该值，详见：[如何获取应用的 App ID](/document/uAjLw4CM/ugTN1YjL4UTN24CO1UjN/trouble-shooting/how-to-obtain-app-id)。
...
<md-alert type="warn">请务必注意本接口仅支持[获取 user_access_token](/document/uAjLw4CM/ukTMukTMukTM/authentication-management/access-token/get-user-access-token)和[刷新 user_access_token](/document/uAjLw4CM/ukTMukTMukTM/authentication-management/access-token/refresh-user-access-token)接口返回的 `refresh_token`</md-alert>
...
1. 在[获取授权码](/document/common-capabilities/sso/api/obtain-oauth-code)时通过 `scope` 参数授权了 `contact:user.base:readonly contact:contact.base:readonly contact:user.employee:readonly` 三个权限。
...
- 当前参数不能传入未授权的权限（即[获取授权码](/document/common-capabilities/sso/api/obtain-oauth-code)时用户已授权范围外的其他权限），否则接口调用会报错，返回错误码 20068。
...
<md-table>
    ...
    <md-tbody>
        ...
        <md-tr>
            ...
            <md-td>应用认证失败，请检查提供的 `client_id` 与 `client_secret` 是否正确。获取方式参见 [如何获取应用的 App ID](/document/uAjLw4CM/ugTN1YjL4UTN24CO1UjN/trouble-shooting/how-to-obtain-app-id)。</md-td>
        </md-tr>  
...
请注意本接口仅支持 v2 版本接口下发的 `refresh_token`
      ...
      <md-tr>
            ...
            <md-td>`refresh_token` 已过期，请[重新发起授权流程](/document/uAjLw4CM/ukTMukTMukTM/authentication-management/access-token/get-user-access-token)以获取新的 `refresh_token`</md-td>
        </md-tr>
      ...
      <md-tr>
            ...
            <md-td>内部服务错误，请稍后重试，如果持续报错请联系[技术支持](https://applink.larksuite.com/TLJpeNdW)</md-td>
        </md-tr>
      ...
      <md-tr>
            ...
            <md-td>`refresh_token` 已被撤销，请[重新发起授权流程](/document/uAjLw4CM/ukTMukTMukTM/authentication-management/access-token/get-user-access-token)以获取新的 `refresh_token`</md-td>
        </md-tr>
      ...
      <md-tr>
            ...
            <md-td>无效的 `scope` 列表，其中存在用户未授权的权限。当前接口 `scope` 参数传入的权限必须是[获取授权码](/document/common-capabilities/sso/api/obtain-oauth-code)时 `scope` 参数值的子集。
...
例如，在获取授权码时，用户授权了权限 A、B，则当前接口 `scope` 可传入的值只有权限 A、B，若传入权限 C 则会返回当前错误码。 </md-td>
      ...
      <md-tr>
            ...
            <md-td>请[重新发起授权流程](/document/uAjLw4CM/ukTMukTMukTM/authentication-management/access-token/get-user-access-token)以获取新的 `refresh_token`</md-td>
        </md-tr>
