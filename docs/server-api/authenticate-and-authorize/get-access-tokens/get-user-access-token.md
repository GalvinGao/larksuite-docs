...
**示例值：**`baBqE5um9LbFGDy3X7LcfxQX1sqpXlwy`
        ...
        <md-tr level="0">
            ...
            <md-td> 是</md-td>
            <md-td>
授权码，详见[获取授权码](/document/common-capabilities/sso/api/obtain-oauth-code)。
...
<md-alert type="tip">建议使用该字段以确定 `user_access_token` 的过期时间，不要硬编码有效期</md-alert>
        ...
        <md-tr level="0">
            ...
            <md-td>              
用于刷新 `user_access_token`，详见[刷新 user_access_token](/document/uAjLw4CM/ukTMukTMukTM/authentication-management/access-token/refresh-user-access-token)。该字段仅在请求成功且用户授予 `offline_access` 权限时返回。
...
<md-alert type="tip">建议在到期前调用[刷新 user_access_token](/document/uAjLw4CM/ukTMukTMukTM/authentication-management/access-token/refresh-user-access-token) 接口获取新的 `refresh_token`。</md-alert> 
            </md-td>
        </md-tr>
...
例如，在获取授权码时，用户授权了权限 A、B，则当前接口 `scope` 可传入的值只有权限 A、B，若传入权限 C 则会返回当前错误码。     
      ...
      <md-tr>
            ...
            <md-td>无效的 `redirect_uri`，请确保 `redirect_uri` 与[获取授权码](/document/common-capabilities/sso/api/obtain-oauth-code)时传入的 `redirect_uri` 保持一致</md-td>
        </md-tr>
