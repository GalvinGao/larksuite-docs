...
| 200 | 20005 | The user access token passed is invalid. Please check the value | 提供的 `user_access_token` 无效，请使用有效的 `user_access_token`。相关文档：[获取 user_access_token](/document/uAjLw4CM/ukTMukTMukTM/authentication-management/access-token/get-user-access-token)。 |
