---
document_id: '7542830025663466929'
directory_id: '7540874596842933681'
title: 重置登录密码
full_path: /uAjLw4CM/ukTMukTMukTM/passport-v1/password/update
breadcrumb:
- Server API
...
updated_at: 2026-08-14T09:03:12Z
source_url: https://open.larksuite.com/document/uAjLw4CM/ukTMukTMukTM/passport-v1/password/update
---

# 重置登录密码

当用户忘记密码、密码已过期或账号存在安全风险时，管理员可以为用户重置密码。{尝试一下}(url=/api/tools/api_explore/api_explore_config?project=passport&version=v1&resource=password&method=update)
...
| 基本 |  |
| --- | --- |
| HTTP URL | https://open.larksuite.com/open-apis/passport/v1/password |
| HTTP Method | PUT |
| 接口频率限制 | [10 次/秒](/document/ukTMukTMukTM/uUzN04SN3QjL1cDN) |
| 支持的应用类型 | <md-app-support types="custom"></md-app-support> |
| 权限要求<br><md-tooltip type="info">调用该 API 所需的权限。开启其中任意一项权限即可调用</md-tooltip> | <md-perm name="passport:password:full" desc="密码管理" support_app_types="custom" tags="">密码管理</md-perm> |
...
<md-dt-tr level="0">
	<md-dt-td>
	<md-text type="field-name" >require_reset</md-text>
	</md-dt-td>
	<md-dt-td>
...
| HTTP状态码 | 错误码 | 描述 | 排查建议 |
| --- | --- | --- | --- |
| 400 | 1082003 | reach reset password num limit | 已超过对同一用户可重置次数的上限，请次日再进行尝试 |
| 400 | 1088002 | password format is not valid | 密码格式错误。请修改密码格式后再尝试。需要重置的密码参数，不少于8个字符，字母、数字和符号，至少三选二 |
| 400 | 1089004 | identity status is abnormal | 未加入和已离职的用户不可重置密码，请确认用户状态 |
| 400 | 1084005 | param is invalid | 参数错误。你需要检查输入参数是否有问题，如果仍然无法解决，请咨询[技术支持](https://applink.larkoffice.com/TLJpeNdW) |
