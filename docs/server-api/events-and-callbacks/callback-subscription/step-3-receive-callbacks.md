---
document_id: '7491263125976858636'
directory_id: '7394375017649569798'
title: 步骤三：接收回调
full_path: /uAjLw4CM/ukTMukTMukTM/event-subscription-guide/callback-subscription/receive-and-handle-callbacks
breadcrumb:
- Server API
...
updated_at: 2025-04-11T02:05:08Z
source_url: https://open.larksuite.com/document/uAjLw4CM/ukTMukTMukTM/event-subscription-guide/callback-subscription/receive-and-handle-callbacks
---
...
当你的业务服务器接收到来自Lark开放平台推送的回调时，你可以通过以下方式确保请求来自Lark开放平台。

:::warning
本文提供的安全校验不适用于  **消息卡片回传交互（旧）**（card.action.trigger_v1）回调。如果你需要为 **消息卡片回传交互（旧）** 配置安全校验，则需要参考[配置回调请求地址](/document/ukTMukTMukTM/uYzMxEjL2MTMx4iNzETM)。
:::
...
如果你在Lark应用内配置了 Encrypt Key 加密策略，则业务服务器接收到回调请求后，需要进行回调解密。
