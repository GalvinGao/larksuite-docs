---
document_id: '7612994308748709300'
directory_id: '7516376096637272070'
title: 使用长连接接收事件
full_path: /ukTMukTMukTM/uYDNxYjL2QTM24iN0EjN/event-subscription-configure-/use-websocket
breadcrumb:
- Server API
...
updated_at: 2026-03-12T14:11:48Z
source_url: https://open.larksuite.com/document/ukTMukTMukTM/uYDNxYjL2QTM24iN0EjN/event-subscription-configure-/use-websocket
---

# 使用长连接接收事件

长连接是[Lark SDK](/document/ukTMukTMukTM/uETO1YjLxkTN24SM5UjN) 内提供的能力，你可以在本地服务器集成Lark SDK， 与开放平台建立一条 WebSocket 全双工通道（你的服务器需要能够访问公网）。后续当应用订阅的事件发生时，开放平台会通过该通道向你的服务器发送事件消息。
...
- 长连接模式仅支持企业自建应用。商店应用事件订阅方式参考[将事件发送至开发者服务器](/document/ukTMukTMukTM/uYDNxYjL2QTM24iN0EjN/event-subscription-configure-/choose-a-subscription-mode/send-notifications-to-developers-server)。
...
以下提供了Lark SDK 的 Golang、Python、Java、Node.js 示例代码，代码内以[接收消息](/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/message/events/receive)事件为例进行配置。通过Lark SDK 除了可以实现事件订阅，还可以实现 API 调用、订阅回调，关于 SDK 详细介绍参见[服务端 SDK](/document/ukTMukTMukTM/uETO1YjLxkTN24SM5UjN)。
...
1. 通过 eventHandler 的 OnXXXX() 方法处理不同的事件。上述示例中分别监听了「接收消息 v1.0」和「接收消息 v2.0」两个事件。
   ...
   **说明**：开放平台提供的事件包括 v1.0 和 v2.0 两个版本。两个版本的结构不同。在处理事件时，可通过[事件列表](/document/ukTMukTMukTM/uYDNxYjL2QTM24iN0EjN/event-list)文档或在开发者后台[添加事件](/document/ukTMukTMukTM/uYDNxYjL2QTM24iN0EjN/event-subscription-configure-/subscription-event-case)时，获取事件的类型与版本信息。

   ![](//sf16-sg.larksuitecdn.com/obj/open-platform-opendoc-sg/4dbc1cd97d95f1af949354ce3c350461_4PX46ts1Hs.png?height=380&lazyload=true&maxWidth=600&width=1666)
...
例如，使用 `onP2MessageReceiveV1` 注册接收消息事件回调时，`P2`便对应的是 v2.0 版本事件结构。
   ...
   ![](//sf16-sg.larksuitecdn.com/obj/open-platform-opendoc-sg/8bbadcadbadcf70bb9cb4c44afea25cd_z7bPrzvx1l.png?height=286&lazyload=true&maxWidth=600&width=1598)
   
1. 通过 larkws.NewClient() 初始化长连接客户端，必填参数为应用的 APP_ID 和 APP_SECRET，需登录[开发者后台](https://open.larksuite.com/app)，在应用详情页的 **凭证与基础信息** > **应用凭证** 区域获取。
    
    ![](//sf16-sg.larksuitecdn.com/obj/open-platform-opendoc-sg/f7f89950be7e57c2760a8b5b1f5e17c9_NouE7PqEkQ.png?height=524&lazyload=true&maxWidth=600&width=3594)
1. 可选参数传入 eventHandler，同时可设置日志级别。
1. 通过 cli.Start() 启动客户端，如连接成功，控制台会打印 "connected to wss://xxxxx"，主线程将阻塞，直到进程结束。
   
   ![](//sf16-sg.larksuitecdn.com/obj/open-platform-opendoc-sg/b1b3d6b4d3a724fa7408f5c71a11d19e_ysuov6r3Bn.png?height=338&lazyload=true&maxWidth=600&width=2286)
...
2. 通过 event_handler 的 `register_{事件版本}_{事件类型 或 customized_event}` 方法处理不同的事件。
    ...
    在开发者后台应用详情页添加事件时（具体操作可参见[添加事件](/document/ukTMukTMukTM/uYDNxYjL2QTM24iN0EjN/event-subscription-configure-/subscription-event-case)），可获取事件的类型与版本信息。

	![](//sf16-sg.larksuitecdn.com/obj/open-platform-opendoc-sg/4dbc1cd97d95f1af949354ce3c350461_bidnBwXPPi.png?height=380&lazyload=true&maxWidth=600&width=1666)
    ...
    **v1.0 版本的事件**
	...
	- 使用 `.register_p1_customized_event("这里填入你要自定义订阅的事件类型，例如 out_approval", do_message_event)` 处理事件：
    	...
    	- customized_event 方法内需要传入事件 Key（String 类型），例如[外出审批](/document/ukTMukTMukTM/uIDO24iM4YjLygjN/event/out-of-office)的事件类型为 out_approval。
...
3. 通过 lark.ws.Client() 初始化长连接客户端，必填参数为应用的 APP_ID 和 APP_SECRET，需登录[开发者后台](https://open.larksuite.com/app)，在应用详情页的 **凭证与基础信息** > **应用凭证** 区域获取。
    
    ![](//sf16-sg.larksuitecdn.com/obj/open-platform-opendoc-sg/f7f89950be7e57c2760a8b5b1f5e17c9_QVldl2kW9x.png?height=524&lazyload=true&maxWidth=600&width=3594)
...
1. 通过 new Client.Builder() 初始化长连接客户端，必填参数为应用的 APP_ID 和 APP_SECRET，需登录[开发者后台](https://open.larksuite.com/app)，在应用详情页的 **凭证与基础信息** > **应用凭证** 区域获取。

     ![](//sf16-sg.larksuitecdn.com/obj/open-platform-opendoc-sg/f7f89950be7e57c2760a8b5b1f5e17c9_1BhhLr3htD.png?height=524&lazyload=true&maxWidth=600&width=3594)
     
1. 可选参数传入 *EVENT_HANDLER*，同时可设置日志级别，可在项目的日志框架（log4j2、logback等）配置中修改。
1. 通过 cli.start() 启动客户端，如连接成功，控制台会打印 "connected to wss://xxxxx"，主线程将阻塞，直到进程结束。
   
   ![](//sf16-sg.larksuitecdn.com/obj/open-platform-opendoc-sg/90d8aab594e3fa8ca22bf4b291723efd_8BitTMiPGo.png?height=176&lazyload=true&maxWidth=600&width=2992)
...
1. 初始化 WSClient。
    ...
    1. 在 baseConfig 中，传入应用的 appId、appSecret。获取方式：登录[开发者后台](https://open.larksuite.com/app)，在应用详情页的 **凭证与基础信息** 页面内，获取应用凭证（包括 App ID 和 App Secret）。
        
        ![](//sf16-sg.larksuitecdn.com/obj/open-platform-opendoc-sg/e78da7b6e674f8ab77a16313123d60b0_I9ZxXgd9Qe.png?height=356&lazyload=true&maxWidth=600&width=2536)
...
2. 基于 Client 完成初始化时，实例上的 start 方法用于启动长连接客户端。
    ...
    如下示例代码部分，需要将 `'im.message.receive_v1'` 替换为需要处理的事件的 **事件类型**。**事件类型** 可参考指定事件的文档，例如，查阅[接收消息](/document/uAjLw4CM/ukTMukTMukTM/reference/im-v1/message/events/receive)事件文档可知，该事件的事件类型为 **im.message.receive_v1**。
...
2. 登录[开发者后台](https://open.larksuite.com/app)，选择一个企业自建应用。
...
4. 编辑订阅方式，选择 **使用长连接接收事件**，并保存。
    ...
    ![image.png](//sf16-sg.larksuitecdn.com/obj/open-platform-opendoc-sg/2c65e52eb0a5b85cd1820812ed42a914_qEgvsUlzcI.png?height=790&lazyload=true&width=1726)

## 常见问题

配置过程中如遇问题，可参考[常见问题](/document/uAjLw4CM/ukTMukTMukTM/event-subscription-guide/event-subscriptions/faq)排查解决。
