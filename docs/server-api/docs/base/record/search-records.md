---
document_id: '7328235243219124230'
directory_id: '7072190414392360966'
title: 查询记录
full_path: /uAjLw4CM/ukTMukTMukTM/reference/bitable-v1/app-table-record/search
breadcrumb:
- Server API
- Docs
- Base
...
updated_at: 2026-03-12T13:49:08Z
source_url: https://open.larksuite.com/document/uAjLw4CM/ukTMukTMukTM/reference/bitable-v1/app-table-record/search
---
...
若多维表格开启了高级权限，你需确保调用身份拥有多维表格的可管理权限，否则可能出现调用成功但返回数据为空的情况。了解具体步骤，参考[如何为应用或用户开通文档权限](/document/ukTMukTMukTM/uczNzUjL3czM14yN3MTN#16c6475a)。
...
<md-table>
  ...
  <md-tbody>
    <md-tr>
      ...
      <md-td>https://open.larksuite.com/open-apis/bitable/v1/apps/:app_token/tables/:table_id/records/search</md-td>
    </md-tr>
    <md-tr>
...
<md-dt-tr level="0">
	...
	<md-dt-td>
	包含条件筛选信息的对象。了解 filter 填写指南和使用示例（如怎样同时使用 `and` 和 `or` 逻辑链接词），参考[记录筛选参数填写指南](/document/uAjLw4CM/ukTMukTMukTM/reference/bitable-v1/app-table-record/record-filter-guide)。
	</md-dt-td>
</md-dt-tr>
...
<md-enum-item key="isNot" >不等于（不支持日期字段，了解如何查询日期字段，参考[日期字段填写说明](/document/uAjLw4CM/ukTMukTMukTM/reference/bitable-v1/app-table-record/record-filter-guide#29d9dc89)）</md-enum-item>
...
<md-dt-tr level="2">
	...
	<md-dt-td>
	条件的值，可以是单个值或多个值的数组。不同字段类型和不同的 operator 可填的值不同。详情参考[字段目标值（value）填写说明](/document/uAjLw4CM/ukTMukTMukTM/reference/bitable-v1/app-table-record/record-filter-guide#3e0fd644)。
...
<md-dt-tr level="1">
	...
	<md-dt-td>
	数组类型。record 结果。了解 record 数据结构，参考[数据结构](/document/uAjLw4CM/ukTMukTMukTM/bitable/development-guide/bitable-structure)。
	</md-dt-td>
</md-dt-tr>
        ...
        "items":[
            {
                ...
                "fields":{
                    ...
                    "群组":[
                        {
                            "avatar_url":"https://internal-api-lark-file.lark-boe.cn/static-resource/v1/v2_c8d2cd50-ba29-476f-b7f1-5b5917cb18ej~?image_size=72x72&amp;cut_type=&amp;quality=&amp;format=jpeg&amp;sticker_format=.webp",
...
- 如果多维表格的 URL 以 ==**larksuite.com/wiki**== 开头，你需调用知识库相关[获取知识空间节点信息](/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/wiki-v2/space/get_node)接口获取多维表格的 app_token。当 `obj_type` 的值为 `bitable` 时，`obj_token` 字段的值才是多维表格的 `app_token`</md-td>
</md-tr>
...
<md-tr>
  ...
  <md-td>filter 参数错误。请参考[记录过滤参数填写指南](/document/uAjLw4CM/ukTMukTMukTM/reference/bitable-v1/app-table-record/record-filter-guide)了解如何填写 filter 参数。</md-td>
</md-tr>
...
<md-tr>
  ...
  <md-td>field_names 参数错误。请检查接口中字段名称和多维表格中的字段名称是否完全匹配。如果难以排查，建议你调用[列出字段](/document/uAjLw4CM/ukTMukTMukTM/reference/bitable-v1/app-table-field/list)接口获取字段名称，因为根据表格页面的 UI 名称可能会忽略空格、换行或特殊符号等差异</md-td>
</md-tr>
...
<md-tr>
  ...
  <md-td>字段名称不存在。请检查：
- 接口中字段名称和多维表格中的字段名称是否完全匹配。如果难以排查，建议你调用[列出字段](/document/uAjLw4CM/ukTMukTMukTM/reference/bitable-v1/app-table-field/list)接口获取字段名称，因为根据表格页面的 UI 名称可能会忽略空格、换行或特殊符号等差异。
