---
document_id: '7444817991944781836'
directory_id: '7071184286824775685'
title: 文档常见问题
full_path: /ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/faq
breadcrumb:
- Server API
- Docs
- Document
...
updated_at: 2024-12-05T06:56:04Z
source_url: https://open.larksuite.com/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/faq
---

# 文档常见问题

## 1.  如何插入带内容的表格（table）？
调用[创建嵌套块](/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/document-block-descendant/create)接口，在指定的 Parent Block 下创建 Table Block
...
![image.png](//sf16-sg.larksuitecdn.com/obj/open-platform-opendoc-sg/d5b6bfc6c86b2db19a5c49200e812c97_gOGa74B4oH.png?height=310&lazyload=true&width=1670)

## 2. 如何插入电子表格（sheet）并往单元格填充内容？

1. 调用 [创建块](/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/document-block-children/create) 接口，在指定 Parent Block 下创建 Sheet Block，指定电子表格的行数量和列数量。
    ```bash
...
2. 返回的 `sheet.token` 的值为电子表格的 token 和电子表格工作表的 ID 的组合。你可继续调用[电子表格相关接口](/document/ukTMukTMukTM/uATMzUjLwEzM14CMxMTN/overview)继续操作该表格。以下示例展示在该电子表格中写入数据。
    ```bash
...
调用 [创建块](/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/document-block-children/create) 接口，在指定的 Parent Block 下创建 Image Block：
...
调用 [上传图片素材](/document/uAjLw4CM/ukTMukTMukTM/reference/drive-v1/media/upload_all) 接口，使用步骤一返回的 Image BlockID 作为 `parent_node` 上传素材：
...
调用 [更新块](/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/document-block/patch) 或 [批量更新块](/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/document-block/batch_update) 接口，指定 `replace_image` 操作，将步骤二返回的图片素材 ID 设置到对应的 Image Block，以更新块为例：
...
调用 [创建块](/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/document-block-children/create) 接口，在指定 Parent Block 下创建 File Block：
...
```json
{
    ...
    "data": {
        "children": [
            {
                "block_id": "doxcnIfCrxq7MlhDbj8xCXmPXgf", // View BlockID
                "block_type": 33,
                "children": [
                    "doxcn1Bx1WOlcqzLqTD2UUYiA7g" // File BlockID
                ],
...
调用[上传图片素材](/document/uAjLw4CM/ukTMukTMukTM/reference/drive-v1/media/upload_all)接口，使用步骤一返回的 File BlockID 作为 `parent_node` 上传素材：
...
--form 'parent_node="doxcn1Bx1WOlcqzLqTD2UUYiA7g"' \ # File BlockID
--form 'size="xxx"' # 文件大小
```
...
调用 [更新块](/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/document-block/patch) 或 [批量更新块](/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/document-block/batch_update) 接口，指定 `replace_file` 操作，将步骤二返回的素材 ID 设置到对应的 File Block，以更新块为例：
...
@用户是 Text Block 中的一个内容实体。如果要 @某个用户，可以调用 [创建块](/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/document-block-children/create)接口，在指定父亲 Block 下创建 Text Block，并在 Text Block 中指定要 @ 的用户 ID：
```json
...
```
如要向文档插入一个公式，可调用[创建块](/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/document-block-children/create)接口，请求体示例如下：
...
调用 [创建块](/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/document-block-children/create)接口，其中路径参数 `block_id` 填写 Callout BlockID，请求体 `children` 填充高亮块的内容。例如，在高亮块内容的第一行插入文本块：
```json
...
调用 [创建块](/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/document-block-children/create) 接口，在指定 Parent block 下创建 Grid block，该 Grid 共计有两列。
...
调用[创建块](/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/document-block-children/create)接口，在指定 Grid Column Block 下插入一个文本 Block。
...
1. 调用[获取文档所有块](/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/document-block/list)接口，分页获取文档所有块的富文本内容。
    ```bash
...
2. 在返回数据中：
   ...
   你可基于图片和文件的 token，调用[下载素材](/document/uAjLw4CM/ukTMukTMukTM/reference/drive-v1/media/download)接口下载对应的图片和文件。
## 10. 如何将 markdown 格式的内容写进Lark在线文档？

目前没有接口支持将 markdown 文本写入Lark在线文档，仅支持将 markdown 文件转为Lark在线文档，详情参考[导入文件概述](/document/uAjLw4CM/ukTMukTMukTM/reference/drive-v1/import_task/import-user-guide)。

## 11. 服务端 OpenAPI 接口限频阈值是多少？

具体请查阅对应接口文档，比如 [更新块的内容](/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/document-block/patch)接口频率限制为每个应用 3 次/秒。
## 12. 文档 OpenAPI 支持哪些类型 Block？

具体请查阅 [块的数据结构](/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/data-structure/block) 中 BlockType  小节。

 ## 13. 新创建的文档还没有 Block，该如何添加 Block ？

新创建的文档有 Block，该 Block 为 Page Block。

创建空文档成功后，接口会返回 `document_id`，`document_id`也是该文档页面块（Page Block）的 `block_id`，因此你可以通过指定 `document_id` 调用 [创建块](/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/document-block-children/create)接口来添加 Block。

## 14. 获取文档所有块接口是按什么顺序返回 Block 的？

[获取文档所有块](/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/document-block/list)接口返回的 items 是一个 [1,N] (N>=1) 的 Block 数组。数组中元素的次序按文档内容先序遍历结果进行排列，其中索引为 0 的元素是文档根节点。

![image.png](//sf16-sg.larksuitecdn.com/obj/open-platform-opendoc-sg/f2781a250e0532e5d09fed46574eecd0_bnsLLioGHI.png?height=436&lazyload=true&width=1123)

以上图为例，其 Blocks 先序遍历结果为：

![image.png](//sf16-sg.larksuitecdn.com/obj/open-platform-opendoc-sg/83dda0898d4d82bc4cb8e4b71c7e0544_m1p0yCU21p.png?height=164&lazyload=true&width=923)

 ## 15. 如何更新文档标题？

文档中的 Block 之间是树状关系，树的根 Block 是 Page Block，文档标题是 Page Block 的文本属性，并且 Page Block 的 BlockID 就是文档的 Token，因此若要更新文档标题，请调用 [更新块的内容](/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/document-block/patch)接口，并指定为 `update_text_elements` 或 `update_text` 操作，其中请求的路径参数：
...
模板其实也是一篇文档，可以通过其链接中的 `document_id` 调用 [复制文件](/document/uAjLw4CM/ukTMukTMukTM/reference/drive-v1/file/copy) 接口创建出一篇新文档。
...
![image.png](//sf16-sg.larksuitecdn.com/obj/open-platform-opendoc-sg/81ebcbea755a1f07ded26ca4d083ce98_Vof57kO9hb.png?lazyload=true&width=1272&height=324)
...
1. 调用 [获取文档所有块](/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/document-block/list)接口，获取电子表格块的 token。若调用成功，预计将返回以下格式数据。
...
1. 基于步骤一获取的电子表格的唯一标识（spreadsheet_token）和电子表格工作表的唯一标识（sheet_id），调用电子表格的[读取单个范围](/document/ukTMukTMukTM/ugTMzUjL4EzM14COxMTN)接口，获取表格中的数据。了解请求示例和响应体示例请直接参考该接口文档。
