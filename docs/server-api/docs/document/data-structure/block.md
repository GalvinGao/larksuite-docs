---
document_id: '7281558204885909510'
directory_id: '7281236599571759109'
title: 块
full_path: /ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/data-structure/block
breadcrumb:
- Server API
- Docs
- Document
...
updated_at: 2024-04-23T11:38:32Z
source_url: https://open.larksuite.com/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/data-structure/block
---
...
| `comment_ids` | `array(string)` | optional | `-` | 评论 ID 列表。查询评论内容请阅览「[获取评论](/document/uAjLw4CM/ukTMukTMukTM/reference/drive-v1/file-comment/get)」。 |
...
| `wiki_catalog` | `object(WikiCatalog)` | optional | `-` | Wiki 子目录 |
...
|`emoji_id` | `enum(string)` | optional | `-` | EmojiID。高亮块支持以下表情：[Emoji 枚举表](/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/document-docx/docx-v1/emoji)|
...
|`view_type` | `int` | optional | `-` | 视图类型，卡片视图（默认）或预览视图 |
...
|`width` | `int` | optional | `100` | 图片宽度，单位 px |
|`height` | `int` | optional | `100` | 图片高度，单位 px|
|`align` | `int` | optional | `2` | 对齐方式 |
...
|`property.header_row` | `boolean` | optional | `false` | 设置首行为标题行 |
|`property.header_column` | `boolean` | optional | `false` | 设置首列为标题列 |
...
## Table Cell
单元格 Block 是一个容器，他的子 Block 中存储了单元格的内容信息，他的父 Block 是表格 Block。在创建表格或向表格添加行列时，系统会自动创建单元格 Block，并默认生成一个内容为空的文本 Block 作为单元格 Block 的子 Block。
```json
{}
```
| 名称         | 数据类型           | 属性        | 默认值         | 描述        |
...
| `comment_ids` | `array(string)` | optional | `-` | 评论 ID 列表。在创建 Block 时，不支持传入评论 ID；在更新文本 Block 的 Element 时，允许将对应版本已存在的评论 ID 移动到同一个 Block 内的任意 Element 中，但不支持传入新的评论 ID。如需查询评论内容请阅览「[获取评论](/document/uAjLw4CM/ukTMukTMukTM/reference/drive-v1/file-comment/get)」。|
...
如需查询任务详情，请调用 [获取任务详情](/document/uAjLw4CM/ukTMukTMukTM/task-v2/task/get) 接口。
```json
{
    "task_id": string
}
...
| 名称         | 数据类型           | 属性        | 默认值         | 描述        |
| --------- | --------------- | -------   | ----------- | --------- |
| `okr_id`  | `string`  | required | `-` | OKR ID，获取需要插入的 OKR ID 可见[获取用户的 OKR 列表](/document/uAjLw4CM/ukTMukTMukTM/reference/okr-v1/user-okr/list)。
...
| 名称         | 数据类型           | 属性        | 默认值         | 描述        |
| --------- | --------------- | -------   | ----------- | --------- |
|`wiki_token` | `string` | 	optional | `-` | [知识库 token](/document/ukTMukTMukTM/uUDN04SN0QjL1QDN/wiki-v2/space/get_node)，不填时自动填充为当前文档所属知识库节点 token |
