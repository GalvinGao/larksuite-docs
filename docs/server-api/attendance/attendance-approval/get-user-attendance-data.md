---
document_id: '7070902815602622470'
directory_id: '7070770034936070149'
title: 获取用户审批通过数据
full_path: /uAjLw4CM/ukTMukTMukTM/Attendance//task/get-user-attendance-data2
breadcrumb:
- Server API
- Attendance
- Attendance Approval
...
source_url: https://open.larksuite.com/document/uAjLw4CM/ukTMukTMukTM/Attendance//task/get-user-attendance-data2
---
...
|名称|类型|必填|描述|
|---|---|---|---|
|employee_type|string|是|请求体中的 user_ids 的员工工号类型，必选字段，可用值：【employee_id（员工 employee ID，即Lark管理后台 > 组织架构 > 成员与部门 > 成员详情中的用户 ID），employee_no（员工工号，即Lark管理后台 > 组织架构 > 成员与部门 > 成员详情中的工号）】，示例值："employee_id"|
### 请求体
|名称|类型|必填|描述|
|---|---|---|---|
...
|&emsp;&emsp;&emsp;∟unit|int|外出时长单位，可用值：【1：上半天；2：下半天；3：全天；4：小时。】|
|&emsp;&emsp;&emsp;∟interval|int|外出时长（单位秒）|
...
|&emsp;&emsp;&emsp;∟unit|int|假期时长单位，可用值：【1（天），0（小时），4（分钟）】|
|&emsp;&emsp;&emsp;∟start_time|string|开始时间，时间格式为 yyyy-MM-dd HH:mm:ss|
|&emsp;&emsp;&emsp;∟end_time|string|结束时间，时间格式为 yyyy-MM-dd HH:mm:ss|
...
|&emsp;&emsp;&emsp;∟type|int|加班规则类型，可用值：【0（不关联加班规则），1（调休），2（加班费）】|
|&emsp;&emsp;&emsp;∟start_time|string|开始时间，时间格式为 yyyy-MM-dd HH:mm:ss|
|&emsp;&emsp;&emsp;∟end_time|string|结束时间，时间格式为 yyyy-MM-dd HH:mm:ss|
