---
document_id: '6975751842354331653'
directory_id: '6975751873563557894'
title: 获取打卡流水记录
full_path: /uAjLw4CM/ukTMukTMukTM/Attendance//GetCardSwipeHistory
breadcrumb:
- Server API
- Attendance
...
- API Reference
- Task
- Get Attendance Flow Records
document_type: GuideDocumentType
updated_at: 2022-03-03T15:54:29Z
source_url: https://open.larksuite.com/document/uAjLw4CM/ukTMukTMukTM/Attendance//GetCardSwipeHistory
---

# 获取打卡流水记录
:::html
<md-alert type="error">
为了更好地提升接口文档的的易理解性，我们对文档进行了升级，请尽快迁移至[新版本>>](/document/uAjLw4CM/ukTMukTMukTM/reference/attendance-v1/user_flow/get)
</md-alert>
:::
...
|名称|类型|必填|描述|
|---|---|---|---|
|user_flow_id|string|是|打卡流水记录 ID，示例值："6708236686834352397"|
### 查询参数
|名称|类型|必填|描述|
|---|---|---|---|
|employee_type|string|是|请求体中的 user_id 和 creator_id 的员工工号类型，可用值：【employee_id（员工的 employeeId），employee_no（员工工号）】，示例值：“employee_id”|
## 响应
### 响应体
|名称|类型|描述|
|---|---|---|
...
|msg|string|错误描述|
|data|-|-|
|&emsp;∟user_id|string|员工工号|
|&emsp;∟creator_id|string|打卡记录创建者的 employee_no|
...
|&emsp;∟type|int|记录生成方式，可用值：【0（用户自己打卡），1（管理员修改），2（用户补卡），3（系统自动生成），4（下班免打卡），5（考勤机打卡），6（极速打卡），7（考勤开放平台导入），8（Lark自研考勤机），9（Lark门禁考勤机）】|
|&emsp;∟photo_urls|string\[\]|打卡照片列表|
|&emsp;∟device_id|string|手机打卡设备ID|
### 响应体示例
```json
{
    "code": 0,
