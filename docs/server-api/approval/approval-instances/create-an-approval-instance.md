---
document_id: '7122386910857560070'
directory_id: '7122028361539026950'
title: 创建审批实例
full_path: /uAjLw4CM/ukTMukTMukTM/reference/approval-v4/instance/create
breadcrumb:
- Server API
- Approval
- Approval instances
...
updated_at: 2025-10-13T07:10:59Z
source_url: https://open.larksuite.com/document/uAjLw4CM/ukTMukTMukTM/reference/approval-v4/instance/create
---
...
<md-table>
  ...
  <md-tbody>
    <md-tr>
      ...
      <md-td>https://open.larksuite.com/open-apis/approval/v4/instances</md-td>
    </md-tr>
    <md-tr>
...
- 登录审批管理后台，在指定审批定义的 URL 中获取，具体操作参见[什么是 Approval Code](/document/uAjLw4CM/ukTMukTMukTM/reference/approval-v4/approval/overview-of-approval-resources#8151e0ae)。
...
<md-dt-tr level="0">
	...
	<md-dt-td>
	审批发起人的 user_id，与 open_id 必须传入其中一个。如果传入了 user_id 则优先使用 user_id。获取方式参考[如何获取用户的 User ID](/document/uAjLw4CM/ugTN1YjL4UTN24CO1UjN/trouble-shooting/how-to-obtain-user-id)。
...
<md-dt-tr level="0">
	...
	<md-dt-td>
	审批发起人的 open_id，与 user_id 必须传入其中一个。如果传入了 user_id 则优先使用 user_id。获取方式参考[如何获取用户的 Open ID](/document/uAjLw4CM/ugTN1YjL4UTN24CO1UjN/trouble-shooting/how-to-obtain-openid)
...
<md-dt-tr level="0">
	...
	<md-dt-td>
	审批发起人所属部门 ID。如果用户只属于一个部门，可以不填。如果用户属于多个部门，不填值则默认选择部门列表第一个部门。获取方式参见[部门 ID](/document/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/department/field-overview#9c02ed7a)。
...
<md-dt-tr level="0">
	...
	<md-dt-td>
	填写的审批表单控件值，JSON 数组，传值时需要压缩转义为字符串。各控件值的参数说明参考[审批实例表单控件参数](/document/uAjLw4CM/ukTMukTMukTM/reference/approval-v4/instance/approval-instance-form-control-parameters)。
...
<md-dt-tr level="1">
	...
	<md-dt-td>
	节点的 node_id 或 custom_node_id，可调用 [查看指定审批定义](/document/uAjLw4CM/ukTMukTMukTM/reference/approval-v4/approval/get) 接口，从接口返回的 node_list 参数中获取。
...
<md-dt-tr level="1">
	...
	<md-dt-td>
	审批人列表，需传入用户 user_id。获取方式参考[如何获取用户的 User ID](/document/uAjLw4CM/ugTN1YjL4UTN24CO1UjN/trouble-shooting/how-to-obtain-user-id)。
...
<md-dt-tr level="1">
	...
	<md-dt-td>
	审批人列表，需传入用户 open_id。获取方式参考[如何获取用户的 Open ID](/document/uAjLw4CM/ugTN1YjL4UTN24CO1UjN/trouble-shooting/how-to-obtain-openid)。
...
<md-dt-tr level="1">
	...
	<md-dt-td>
	节点的 node_id，可调用 [查看指定审批定义](/document/uAjLw4CM/ukTMukTMukTM/reference/approval-v4/approval/get) 接口，从接口返回的 node_list 参数中获取。
...
<md-dt-tr level="1">
	...
	<md-dt-td>
	抄送人列表，需传入用户 user_id。获取方式参考[如何获取用户的 User ID](/document/uAjLw4CM/ugTN1YjL4UTN24CO1UjN/trouble-shooting/how-to-obtain-user-id)。
...
<md-dt-tr level="1">
	...
	<md-dt-td>
	抄送人列表，需传入用户 open_id。获取方式参考[如何获取用户的 Open ID](/document/uAjLw4CM/ugTN1YjL4UTN24CO1UjN/trouble-shooting/how-to-obtain-openid)。
...
<md-dt-tr level="1">
	...
	<md-dt-td>
	节点 ID 值，可调用 [查看指定审批定义](/document/uAjLw4CM/ukTMukTMukTM/reference/approval-v4/approval/get) 接口，从接口返回的 node_list 参数中获取。
...
- 如果传入的有表单参数（form），则需要检查该参数内传入的表单控件数据是否正确。如果报错信息内包含控件 ID（如 `控件= widget17261088448220001`），可以调用[查看指定审批定义](/document/uAjLw4CM/ukTMukTMukTM/reference/approval-v4/approval/get)或者[获取单个审批实例详情](/document/uAjLw4CM/ukTMukTMukTM/reference/approval-v4/instance/get)接口，获取响应参数 form 值，检索有问题的控件 ID，然后检查该控件的配置是否正确。</md-td>
</md-tr>
...
<md-tr>
  ...
  <md-td>审批定义已停用，请确保当前所用的审批定义已启用后重试。你可以登录[Lark审批管理后台](https://www.larksuite.com/approval/admin/approvalList)，查看相应审批定义是否被停用。

![image.png](//sf16-sg.larksuitecdn.com/obj/open-platform-opendoc-sg/3991337412ec292a8f61d5c57a5d590b_MbHV5E0Qmr.png)</md-td>
</md-tr>
...
2. 降低请求频率，并重试。如果重试仍然报错，请联系[技术支持](https://applink.larksuite.com/TLJpeNdW)。</md-td>
</md-tr>
