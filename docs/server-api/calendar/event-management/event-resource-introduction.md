...
- 关于 `calendar_id`，可参见 [日历资源概述](/document/uAjLw4CM/ukTMukTMukTM/reference/calendar-v4/calendar/introduction)。
- 关于日程组织者，可参见本文的 **日程组织者** 章节。

          
**示例值**："larksuite.com_ZefWrFi6e12ds@group.calendar.larksuite.com"<br>
        </md-td>
</md-tr>
...
- `third_party`：第三方会议链接
- `no_meeting`：无视频会议
- `lark_live`：Lark直播，只读字段，该类型视频会议只能由Lark系统生成。
...
<md-tr>
        ...
        <md-td>
当 `vc_type` 为 `third_party` 时，该字段定义Lark客户端展示的视频会议 Icon 类型。<br>
...
<md-tr>
        ...
        <md-td>
当 `vc_type` 为 `third_party` 时，该字段定义Lark客户端展示的视频会议提醒文案。<br>
...
- `private`：订阅该日程所在日历的用户，无法查看此日程的详情，除非他们拥有该日历 `writer` 或 `owner` 级别的访问控制（ACL）。关于 ACL，请参考 [用户访问控制资源概述](/document/uAjLw4CM/ukTMukTMukTM/reference/calendar-v4/calendar-acl/introduction)。
...
<md-tr>
        ...
        <md-td>
当前身份下，日程所在日历上对应时间段的忙闲状态。相关 API 参见 [查询主日历日程忙闲信息](/document/uAjLw4CM/ukTMukTMukTM/reference/calendar-v4/freebusy/list)。<br>
...
**当前身份**：由请求 API 时的鉴权方式决定，详细说明参见[API 访问凭证概述](/document/ukTMukTMukTM/uMTNz4yM1MjLzUzM)。
        </md-td>
</md-tr>
...
- Lark客户端展示日程颜色时，会将该值映射到预设色板上最相近的颜色。<br>

- 当日程颜色值为 0 或 -1 时，日程颜色展示将跟随日历颜色。关于日历颜色，可参见[日历资源概述](/document/uAjLw4CM/ukTMukTMukTM/reference/calendar-v4/calendar/introduction)。
          
**示例值**：-11034625<br>
          
**当前身份**：由请求 API 时的鉴权方式决定，详细说明参见 [API 访问凭证概述](/document/ukTMukTMukTM/uMTNz4yM1MjLzUzM)<br>
        </md-td>
</md-tr>
...
<md-tr>
        ...
        <md-td>
用于控制Lark客户端日程详情页部分 UI 的自定义信息。<br>
        </md-td>
</md-tr>
...
<md-tr>
        ...
        <md-td>
UI 名称，Lark客户端日程详情页按钮或区域的唯一标识。<br>
