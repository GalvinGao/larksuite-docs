...
<md-alert type="tip" icon="none">开放平台设计了访问凭证（access_token）机制，以确保 API 调用的安全性。详情可参见[获取访问凭证](/document/ukTMukTMukTM/uMTNz4yM1MjLzUzM)。你在实际调用 API 时，需要使用访问凭证来表明当前的身份。</md-alert>
</md-td>
</md-tr>
  
<md-tr>
<md-td>日历</md-td>
<md-td>Lark日历主要分为主日历、共享日历两类。此外也支持同步第三方日历（例如 [Exchange](/document/uAjLw4CM/ukTMukTMukTM/reference/calendar-v4/exchange_binding/introduction)）。
...
**示例值**："larksuite.com_xxxxxxxxxx@group.calendar.larksuite.com"<br>
          
**获取方式**：
- [创建共享日历](/document/uAjLw4CM/ukTMukTMukTM/reference/calendar-v4/calendar/create)
