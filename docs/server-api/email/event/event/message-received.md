---
document_id: '7327563913327165446'
directory_id: '7325375410139971589'
title: 收信通知
full_path: /uAjLw4CM/ukTMukTMukTM/reference/mail-v1/user_mailbox-event/events/message_received
breadcrumb:
- Server API
- Email
- Event
...
updated_at: 2025-04-28T11:38:40Z
source_url: https://open.larksuite.com/document/uAjLw4CM/ukTMukTMukTM/reference/mail-v1/user_mailbox-event/events/message_received
---

# 收信通知

## 前提条件
你需要在应用中配置事件订阅，这样才可以在事件触发时接收到事件数据。了解事件订阅可参见 [事件概述](/document/ukTMukTMukTM/uUTNz4SN1MjL1UzM)。{使用示例}(url=/api/tools/api_explore/api_explore_config?project=mail&version=v1&resource=user_mailbox.event&event=message_received)
...
| 权限要求<br><md-tooltip type="info">订阅该事件所需的权限，开启其中任意一项权限即可订阅</md-tooltip> | <md-perm name="mail:event" desc="订阅邮箱相关事件" support_app_types="custom" tags="">订阅邮箱相关事件</md-perm> |
| 字段权限要求 | <md-alert type="tip" icon="none"><br>该接口返回体中存在下列敏感字段，仅当开启对应的权限后才会返回；如果无需获取这些字段，则不建议申请<br></md-alert><br><md-perm name="mail:user_mailbox.event.mail_address:read" desc="获取事件中的邮箱地址字段" support_app_types="custom" tags="">获取事件中的邮箱地址字段</md-perm> |
