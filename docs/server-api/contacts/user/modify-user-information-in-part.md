---
document_id: '6965400907875188742'
directory_id: '6920532209305042945'
title: 修改用户部分信息
full_path: /uAjLw4CM/ukTMukTMukTM/reference/contact-v3/user/patch
breadcrumb:
- Server API
- Contacts
- User
- Modify user information in part
document_type: ReferenceDocumentType
updated_at: 2024-03-05T07:18:24Z
source_url: https://open.larksuite.com/document/uAjLw4CM/ukTMukTMukTM/reference/contact-v3/user/patch
---

# 修改用户部分信息

该接口用于更新通讯录中用户的字段，未传递的参数不会更新。{尝试一下}(url=/api/tools/api_explore/api_explore_config?project=contact&version=v3&resource=user&method=patch)
...
<md-dt-tr level="2">
	...
	<md-dt-td>
	字段类型为`TEXT`时该参数定义字段值，必填，长度限制为100字符；字段类型为`HREF`时该参数定义网页标题，必填
...
<md-dt-tr level="0">
	...
	<md-dt-td>
	分配给用户的席位ID列表，需开通“分配用户席位”权限。可通过下方接口获取到该租户的可用席位ID，参见[获取企业席位信息](/document/uAjLw4CM/ukTMukTMukTM/tenant-v2/tenant-product_assign_info/query)
...
| 409 | 41003 | user account conflict error | 用户的联系方式属于两个不同的Lark账号，添加失败。建议用户换其它的手机号或邮箱，或是先注销手机号或邮箱对应的帐号，然后再创建或更新，具体注销的流程见[帮助中心](https://www.feishu.cn/hc/zh-CN/articles/360049067831)<br>有关这个错误的详细介绍请参考[通用错误码](/document/ukTMukTMukTM/ugjM14COyUjL4ITN#74588198)中的介绍 |
