---
document_id: '7143913324618006533'
directory_id: '6907567266540847106'
title: Errno错误码
full_path: /uYjL24iN/uAjMuAjMuAjM/errno
breadcrumb:
- Client API
- Web app/Gadget API
- Errno
document_type: GuideDocumentType
updated_at: 2024-07-04T12:56:52Z
source_url: https://open.larksuite.com/document/uYjL24iN/uAjMuAjMuAjM/errno
---
...
| 1501001   | Bluetooth adapter not initialized | 蓝牙适配器未初始化  |蓝牙
| 1501002   | Bluetooth adapter not available | 当前蓝牙适配器不可用  |蓝牙
| 1501003   | Device not found | 没有找到指定设备  |蓝牙
| 1501004   | Connection failed | 连接失败  |蓝牙
| 1501005   | Service not found | 没有找到指定服务  |蓝牙
| 1501006   | Characteristic not found | 没有找到指定特征值  |蓝牙
| 1501007   | Connection lost | 当前连接已断开  |蓝牙
| 1501008   | Operation not available on this characteristic | 当前特征值不支持此操作  |蓝牙
| 1501009   | System error | 系统错误  |蓝牙
| 1501010   | BLE not available on this device | 系统不支持，Android系统版本低于4.3  |蓝牙
| 1501011   | Descriptor not found | 没有找到指定描述符  |蓝牙
| 1501012   | Invalid data | 发送的数据为空或格式错误  |蓝牙
| 1501013   | Operation timed out | 操作超时  |蓝牙
| 1503001   | Not supported by the device/system | 系统或设备不支持  |Beacon
| 1503002   | Search already started | 已经开始搜索  |Beacon
| 1503003   | Search not started yet | 还未开始搜索  |Beacon
| 1503004   | Location service not available | 位置服务不可用  |Beacon
| 1503005   | Bluetooth service not available | 蓝牙服务不可用  |Beacon
...
| 2700001   | Unable to get session  | 获取session失败  |[requestAccess](/document/uYjL24iN/uUzMuUzMuUzM/requestaccess)
| 2700002   | Authorization terminated unexpectedly. Error code: xx; error message:	xx | 授权过程失败, code: xx, msg: xx ( 含义参见 [服务端通用错误码](/document/ukTMukTMukTM/ugjM14COyUjL4ITN) )  |[requestAccess](/document/uYjL24iN/uUzMuUzMuUzM/requestaccess)
...
* 若一个API存在存量的errCode，我们会保持现状，不删除，不修改，如果开发者的存量代码依赖了这些errCode, 可以继续保留这些代码作为对旧版本Lark的兼容性逻辑。
