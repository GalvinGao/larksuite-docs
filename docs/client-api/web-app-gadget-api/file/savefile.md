...
| filePath | string | 否 |  | 文件路径。格式为：ttfile://user/feishu.png, 其中 ttfile://user/为固定格式，feishu.png 为文件名。如果不填则给一个随机路径。**不支持网络地址**<br>**示例值**：ttfile://user/feishu.png<br><md-alert type="tip" icon="none"><br>- Android/iOS 端<br>- 网页应用：Lark[V3.44.0](/document/uYjL24iN/uAjMuAjMuAjM/version-compatibility)及以上版本支持<br>- PC 端：暂不支持<br></md-alert> |
