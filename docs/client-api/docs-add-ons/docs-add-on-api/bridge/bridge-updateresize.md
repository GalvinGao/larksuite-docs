---
document_id: '7270779605450833926'
directory_id: '7270719284443594757'
title: Bridge.updateResize
full_path: /uAjLw4CM/uYjL24iN/docs-add-on/05-api-doc/basic-data-reference---base/Bridge.updateResize
breadcrumb:
- Client API
- Docs Add-ons
- Docs Add-on API
...
source_url: https://open.larksuite.com/document/uAjLw4CM/uYjL24iN/docs-add-on/05-api-doc/basic-data-reference---base/Bridge.updateResize
---
...
:::html
<md-alert type="error">
  - 移动端不支持resize，最好在移动端场景不要调用这个api，移动端可以用updateHeight。
...
DocMiniApp.Bridge.updateHeight({width: 100});
DocMiniApp.Bridge.updateHeight({height: 100});
DocMiniApp.Bridge.updateHeight({resizeType: 'free' });
DocMiniApp.Bridge.updateHeight({width: 100, height: 100, resizeType: 'free' });
DocMiniApp.Bridge.updateHeight({width: 100, height: 100});
```

## Demo示例

[03 云文档小组件快速开发指南](/document/uAjLw4CM/uYjL24iN/docs-add-on/03-cloud-document-widget-quick-development-guide/03-cloud-document-widget-quick-developme) 创建应用后，把下面代码复制粘贴到对应文件
**App.tsx**
```js
...
export default () => {
  ...
  return (
    <div className="demo" ref={domRef}>
      ...
      <div style={{ marginTop: 20 }}>当前宽度是：{size.width}，高度是：{size.height}，类型是：{size.resizeType}</div>
      <div className={`click-update-resize`} onClick={updateResize}>点击变更</div>
...
返回示例
上面的代码实现的效果
![](//sf16-sg.larksuitecdn.com/obj/open-platform-opendoc-sg/46df6d56e175ce8b2208dbd3bcc50e4b_wn6SXBaWyn.gif?lazyload=true&width=2028&height=1022)
